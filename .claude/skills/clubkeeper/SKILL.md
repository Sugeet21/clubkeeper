...
- **Sync project (Phase C Chunks 0–4.3 OWNER-VERIFIED, Chunk 5.0/5.1/5.2 pre-commit LANDED, 1 Jul 2026)** — Write path (Chunk 4.3): 50-row backlog → 50 unique rows in Supabase, per-pushOne 162–475ms, zero watchdog timeouts. Read path (Chunk 5.0): `SyncReader` singleton at `src/db/syncReader.ts` — cursor-driven initial pull across all 9 synced tables (`.eq('club_id', ownerClubId).gt('updated_at', cursor).order('updated_at').limit(BATCH)`), per-table cursor in `settings.pullCursors`. Only `customers` + `canteen_sales` currently wired via `syncReadMapper`; other 7 tables THROW-as-unmapped until Chunk 5.2b. Boot at `<SyncReaderBoot />` in `App.tsx`. Runtime-verified 1 Jul 2026 (dev capture): single `start` + single `initialPull complete` (928ms) despite StrictMode double-init of authStore — Pattern A10 boot-effect invariant proven. Three-client architecture (Pattern S16): `supabase` (owner AUTH only + realtime), `supabasePublic` (anon player-hub RPCs, distinct `storageKey: 'sb-clubkeeper-public'`), `supabaseSync` (owner DATA-PLANE reads + writes, lock-free via `accessToken` option — promoted from write-only in Chunk 5.0). Self-healing drain (Pattern S15). Broken-hook recovery (Chunk 5.2 pre-commit): `NoUserClubIdClaimError` typed error + `SyncReader.deferForRefresh` with teardown-before-register + `hasLoggedClaimGuidance` latch → guidance logs once, retry re-fires on `TOKEN_REFRESHED` with exactly one live listener regardless of refresh cycles (static reviewer APPROVE; runtime toggle deferred). Server-side LWW guard migration (Chunk 5.1) prevents stomping newer peer edits — PENDING prod-apply (see Pending list). Boot-effect authoring rule (Pattern A10): app-shell singletons dep on `session?.user?.id` primitive, NEVER the session object. Next chunk: 5.2b (map the remaining 7 tables in `syncReadMapper`) + realtime subscription integration.
...
- **Chunk 5.2b FIRST-verification requirement — mapper TRANSFORM proof**: The Chunk 5.2 pre-commit runtime capture (1 Jul 2026) pulled 0 rows across the two mapped tables (customers + canteen_sales), so `syncReadMapper`'s TRANSFORM path never executed — only the pull mechanism (cursor pagination, generation guard, DIAG-clean logging) was runtime-proven. Before Chunk 5.2b is declared VERIFIED: seed a real row directly in Supabase for BOTH `customers` and `canteen_sales`, force an `initialPull` (or wait for cursor advance), then inspect the resulting Dexie row shape via IndexedDB. MUST confirm on the pulled row: `*At` fields are numeric epoch ms (createdAt, updatedAt, deletedAt if soft-deleted), `updated_at` / `deleted_at` raw fields are NOT persisted on the Dexie side (only the camelCase epoch-ms versions), `items` (canteen_sales) is an array of objects, `paymentBreakdown` (canteen_sales) is an object not a JSON-string. This is the mapper contract that would silently fail if inverted; runtime proof of the mechanism alone is NOT sufficient.
