...
- **Sync project (Phase C Chunks 0–3 COMPLETE + VERIFIED, Chunk 4 BLOCKED on #109, 25 Jun 2026)** — Multi-device sync + owner auth. DDL deployed; JWT hook patched (#108 closed). Chunk 3 wrappers (syncedCreate/Update/SoftDelete/Batch) smoke-tested via `/__dev/test-outbox` — all 4 PASS confirmed by owner. Chunks 0–3 files: `syncWrappers.ts`, `syncTableMap.ts`, `scheduleDrain.ts` stub, `TestOutbox.tsx` DEV-only page, dev nav button in /settings. All wrappers atomic data+outbox writes in one Dexie tx; Pattern D7 documented top-of-file. **#109 BUG-S13 BLOCKS Chunk 4**: JWT still missing `user_club_id` + `user_role` claims despite hook patch. Next session = dedicated #109 debug (NOT new client code). Diagnosis playbook: run `select public.add_user_meta_to_jwt(...)` direct test → check Postgres logs → try `GRANT EXECUTE TO supabase_auth_admin` → toggle hook off/on. Until claims appear in jwt.io, SyncRunner cannot ship — every Supabase upsert will RLS-403.

## Pending — load-bearing, delete when done

Things that BLOCK something if forgotten. Delete the line the moment it's resolved.

- **Migration: `supabase/migrations/20260625_phase_c_sync_tables.sql`** — 9 sync tables + users_meta + JWT custom-claims hook + clubs.sync_enabled + RLS owner-only + realtime publication grants. Until run, Phase C Chunks 3–7 cannot proceed: the sync engine has nowhere to push to. Post-run, Sugeet must (a) configure Custom Access Token Hook in Auth → Hooks, (b) INSERT his own users_meta row, (c) sign out + in so JWT refreshes. Token `PHASE_C_DDL_DEPLOYED` in chat unblocks Chunk 3.
