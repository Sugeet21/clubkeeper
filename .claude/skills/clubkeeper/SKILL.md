...
- **Sync project (Phase C Chunks 0–3 COMPLETE, Chunk 4 BLOCKED on #109, 25 Jun 2026)** — Multi-device sync + owner auth. DDL deployed; JWT hook patched (#108 closed). Chunk 3: `src/db/syncWrappers.ts` (syncedCreate/Update/SoftDelete + syncedCreateBatch) + `syncTableMap.ts` (snake↔camel) + `scheduleDrain.ts` stub + DEV-only `/__dev/test-outbox` smoke-test page. All wrappers atomic data+outbox writes in one Dexie tx; Pattern D7 documented top-of-file. **#109 BUG-S13 BLOCKS Chunk 4**: JWT still missing `user_club_id` + `user_role` claims despite hook patch. Until Sugeet verifies claims appear in jwt.io payload, SyncRunner cannot be wired — every Supabase upsert will RLS-403.
