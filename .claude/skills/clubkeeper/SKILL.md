...
- **Sync project (Phase C Chunks 0–5.2b OWNER-VERIFIED; Chunk 5.3 COMMITTED, runtime proof pending)** — Write path (Chunk 4.3): 50-row backlog → 50 unique rows in Supabase, zero watchdog timeouts. Read path: `SyncReader` singleton (`src/db/syncReader.ts`) — serialized job queue (Chunk 5.3: FIFO of deduped `pull` jobs + one `apply` job per realtime event, single worker + S15 generation guards), compound per-table cursor in `settings.pullCursors` (`created_at` cursor for append-only `wallet_transactions`, `updated_at` for the other 8), 4 grouped realtime channels per §7.2 on the MAIN `supabase` client (Pattern S22 — subscribe on login after claim resolution, teardown on logout). Chunk 5.3: handlers DIRECT-APPLY `payload.new` (§7.3) — outbox-guard → numeric epoch-ms LWW compare (S17) → tie-break (equal ms yields to remote; `updated_by` NULL from our pushes so the same-user branch is unreachable today) → `fromSupabaseRow` → `put` → monotonic cursor advance (never from a null cursor); DELETE/malformed events fall back to the doorbell `requestPull`. ALL 9 tables mapped bidirectionally (`syncReadMapper` + `syncPayloadMapper`) under the #117 contract (CLOSED): Dexie-side LWW metadata is camelCase epoch-ms `updatedAt`/`deletedAt`, ISO only at the wire (Pattern S17); nested jsonb always real objects. Server-side LWW guard triggers (20260628) + client-field columns (20260702) both verified in prod. `pushOne` soft-delete on `wallet_transactions` throws (append-only). Three-client architecture (Pattern S16): `supabase` (owner AUTH + realtime), `supabasePublic` (anon RPCs), `supabaseSync` (owner data plane, lock-free). Broken-hook recovery via `deferForRefresh` (static APPROVE; runtime proof still pending #116 — DEV `__force_no_claim__` toggle + `window.__supabase` shipped for the test). DEV proof pages `/__dev/test-outbox` + `/__dev/test-sync-reader` (incl. LWW conflict-test buttons). Next: 5.3 runtime proof (conflict test + shape dump + #116 toggle) → 5.4 polling fallback → Chunk 7 queries.ts wrapper cutover.

## Pending — load-bearing, delete when done

Things that BLOCK something if forgotten. Delete the line the moment it's resolved.

- **Chunk 5.2b residual — canteen_sales/session_items pulled-row SHAPE dump never visually inspected**: owner verification (2 Jul 2026) covered 4-channel realtime subscribe, <2s doorbell pull (`fetched 1, applied 1`), and pulled `game_tables` rendering from Dexie (which proves that mapper's TRANSFORM end-to-end). The `/__dev/test-sync-reader` "Dump synced tables" output for `canteen_sales` (items array + paymentBreakdown object, no snake_case keys) and `session_items` was NOT pasted. Low risk (same helper code paths, reviewer-verified statically) but confirm incidentally during Chunk 5.3 testing — a wrong shape there would surface as broken QuickSale/Summary reads on a fresh-device pull. Local Dexie may still hold the 4 `TEST 52B` rows (Supabase side already cleaned) — delete via the console snippet in the 2 Jul changelog entry.
- **Chunk 5.2 pre-commit — broken-hook TOKEN_REFRESHED runtime proof (resolve before prod)**: static reviewer APPROVE on all 8 invariants but the deferForRefresh single-fire path is NOT runtime-proven. Blocker on Phase C prod push. See #116 for the full test-plan. The plumbing is now SHIPPED DEV-gated (Chunk 5.3, 3 Jul 2026): `localStorage.setItem('__force_no_claim__','1')` makes `getOwnerClubIdFromJwt` throw, and `window.__supabase.auth.refreshSession()` is console-callable — no temp source edits needed, just run steps 3–5 of the issue plan and paste the capture. Reader Boot Pattern A10 IS runtime-proven; this is only the broken-hook branch.
