...
16. **STAGING-FIRST for any non-trivial change (deploy law, since 25 Jul 2026 — after the demo-on-live disaster).** THREE environments, TWO databases: **local** (`npm run dev`) + **staging** (`staging` branch → `clubkeeper-git-staging-sugeet-s-projects.vercel.app`) both use the throwaway STAGING Supabase (`tdcvrmttnsyhqxxhfvwj`); **prod** (`main` → `app.handbookhq.in`) is the ONLY thing on the real DB (`vkczmgzujpidbwtzulel`). Flow: (1) code on `staging` branch → (2) push → auto-deploys to staging URL → (3) VERIFY the real flow on staging → (4) ONLY if it passes, `git merge staging → main` + push (that merge IS the promote-to-prod) → (5) post SHA on the issue → (6) OWNER verifies on their real device → (7) owner closes the ticket LAST (Rule F unchanged). **The ticket closes AFTER prod + owner-verify, never before.** DB changes: apply the ONE migration file to staging DB first, test, then apply the SAME file to prod DB (two applies of one file). Trivial/typo/skill-markdown changes may still go straight to `main` (they have no runtime surface — same carve-out as the abbreviated loop). Local `.env.local` points at staging (gitignored). Details: `references/deployment.md` §Staging + `supabase/staging/`.
