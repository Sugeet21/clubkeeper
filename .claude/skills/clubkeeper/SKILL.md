...
- **Sync project (Phase C Chunks 0–5.2b OWNER-VERIFIED, 2 Jul 2026; both LWW-era migrations APPLIED to prod)** — Write path (Chunk 4.3): 50-row backlog → 50 unique rows in Supabase, zero watchdog timeouts. Read path: `SyncReader` singleton (`src/db/syncReader.ts`) — serialized pull queue (insertion-ordered Set + single worker + S15 generation guards), compound per-table cursor in `settings.pullCursors` (`created_at` cursor for append-only `wallet_transactions`, `updated_at` for the other 8), 4 grouped realtime doorbell channels per §7.2 on the MAIN `supabase` client (Pattern S22 — subscribe on login after claim resolution, teardown on logout; handlers call `requestPull`, never apply payloads directly). Owner runtime capture: all 4 channels SUBSCRIBED, SQL-update doorbell caught <2s (`fetched 1, applied 1`), pulled `game_tables` rendering from Dexie on `/tables`. ALL 9 tables mapped bidirectionally (`syncReadMapper` + `syncPayloadMapper`) under the #117 contract (CLOSED): Dexie-side LWW metadata is camelCase epoch-ms `updatedAt`/`deletedAt`, ISO only at the wire (Pattern S17); nested jsonb always real objects. Server-side LWW guard triggers (20260628) + client-field columns (20260702: sessions/bookings `config` jsonb, `canteen_items.stock_enabled`, wallet coin columns, `reference_id` text) both verified in prod. `pushOne` soft-delete on `wallet_transactions` throws (append-only). Three-client architecture (Pattern S16): `supabase` (owner AUTH + realtime), `supabasePublic` (anon RPCs), `supabaseSync` (owner data plane, lock-free). Broken-hook recovery via `deferForRefresh` (static APPROVE; runtime proof still pending #116). DEV proof pages `/__dev/test-outbox` + `/__dev/test-sync-reader`. Next: Chunk 5.3 (READ-path direct-apply LWW handler + tie-break for realtime payloads, replacing the doorbell re-pull where profitable) → 5.4 polling fallback → Chunk 7 queries.ts wrapper cutover.
