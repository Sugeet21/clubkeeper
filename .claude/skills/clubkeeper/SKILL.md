...
- **Sync project (Phase C Chunks 0–4.3 OWNER-VERIFIED; 5.0/5.1/5.2/5.2b LANDED, 2 Jul 2026)** — Write path (Chunk 4.3): 50-row backlog → 50 unique rows in Supabase, zero watchdog timeouts. Read path: `SyncReader` singleton (`src/db/syncReader.ts`) — serialized pull queue (insertion-ordered Set + single worker + S15 generation guards), compound per-table cursor in `settings.pullCursors` (`created_at` cursor for append-only `wallet_transactions`, `updated_at` for the other 8), 4 grouped realtime doorbell channels per §7.2 on the MAIN `supabase` client (Pattern S22 — subscribe on login after claim resolution, teardown on logout; handlers call `requestPull`, never apply payloads directly; direct-apply LWW = Chunk 5.3). ALL 9 tables mapped bidirectionally (`syncReadMapper` + `syncPayloadMapper`, Chunk 5.2b) under the #117 contract: Dexie-side LWW metadata is camelCase epoch-ms `updatedAt`/`deletedAt`, ISO only at the wire (Pattern S17); nested jsonb always real objects. `sessions`/`bookings` ride a `config` jsonb column, `canteen_items` a `stock_enabled` column, wallet coins three new columns — all in migration `20260702_sync_client_fields.sql` (PENDING prod-apply). `pushOne` soft-delete on `wallet_transactions` throws (append-only). Three-client architecture (Pattern S16): `supabase` (owner AUTH + realtime), `supabasePublic` (anon RPCs), `supabaseSync` (owner data plane, lock-free). Broken-hook recovery via `deferForRefresh` (static APPROVE; runtime proof pending #116). DEV proof pages `/__dev/test-outbox` + `/__dev/test-sync-reader`. Chunk 5.2b reviewer verdict: REQUEST_CHANGES → all items fixed. Runtime TRANSFORM proof PENDING (seeds already in Supabase — see Pending). Next: runtime proof → 5.3 LWW direct-apply + tie-break → 5.4 polling fallback → Chunk 7 queries.ts wrapper cutover.
...
- **Chunk 5.2b VERIFICATION — mapper TRANSFORM runtime proof (code LANDED 2 Jul 2026; proof still pending)**: TEST rows are ALREADY SEEDED in production Supabase (2 Jul 2026): customers `fab82788-8ae7-4a6d-be7e-96c1ee5a16c2`, canteen_sales `b858f6c6-3d02-44c3-8d8a-9ba312eadb51`, game_tables `c6008db1-bd6a-41de-94f7-e6ab8bbbc028` (config carries a 2-tier rateCard — proves nested-object parse), session_items `89b270fb-71e1-4a07-9aad-957be149510e`. All names prefixed `TEST 52B`. Proof procedure: `npm run dev` → sign in → `/__dev/test-sync-reader` → 1. Reset pull cursors → 2. Force pull → 3. Dump synced tables → paste the dump. MUST confirm on each pulled row: `*At` fields numeric epoch ms (`createdAt`/`updatedAt`/`addedAt`); NO raw `updated_at`/`deleted_at` keys; `items` + `paymentBreakdown` (canteen_sales) and `rateCard` (game_tables) are real objects/arrays, never JSON strings; game_tables `outOfService === false` (inverse of `is_active: true`). Cleanup after proof: delete the 4 Supabase rows (`delete from <table> where id = '<id>'`) AND the 4 Dexie rows (same ids) — pulls do not hard-delete. `sessions`/`bookings`/`canteen_items`/wallet coin rows CANNOT be proof-seeded until migration 20260702 is applied.
- **Chunk 5.2 pre-commit — broken-hook TOKEN_REFRESHED runtime proof (resolve before prod)**: static reviewer APPROVE on all 8 invariants but the deferForRefresh single-fire path is NOT runtime-proven. Blocker on Phase C prod push. See #116 for the full test-plan (localStorage `__force_no_claim__` toggle + `window.__supabase.auth.refreshSession()` from console). Reader Boot Pattern A10 IS runtime-proven; this is only the broken-hook branch.
- **Migration: `supabase/migrations/20260702_sync_client_fields.sql`** — sessions.config + bookings.config jsonb, canteen_items.stock_enabled, wallet_transactions balance_type/coin_delta/rupee_equivalent + reference_id uuid→text (Chunk 5.2b, refs #112). Until run: read mappers THROW on any sessions/bookings/canteen_items server row (fail-loud; empty tables no-op), coin-row pushes 400 → dead-letter. Verification query in the file footer (expects 6 rows + reference_id data_type = text).
