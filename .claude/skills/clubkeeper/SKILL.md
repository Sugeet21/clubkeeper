...
- **Sync project (Phase C Chunk 0 COMPLETE, 25 Jun 2026)** — Multi-device sync + owner auth (staff login deferred to Phase D). Architecture doc at `references/sync_architecture_v2.md` (v3.2 amendment at top — §4.2 session_items DDL drops invented `canteen_item_id` column, clubs uses `owner_id` not `owner_user_id`). Chunk 0 shipped: Booking.tableId + Booking.consumedSessionId narrowed to string; `_migrationSeq?` added to GameTable/Session/SessionItem/CanteenItem; `SyncTableName` (snake_case Supabase format) exported; OutboxRow.table retyped; v20 .upgrade() bookings .modify() now also remaps consumedSessionId. Next: Phase C Chunk 1 (useCurrentUser hook on top of existing authStore) + Chunk 2 (DDL migration SQL → manual deploy).
