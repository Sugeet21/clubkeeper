...
- **Peak Hour Pricing (#68)** — Phases 1–3 shipped: schema + Settings picker (P1), Canteen card + form field (P2), AddItemBottomSheet + QuickSale chips with amber `PEAK` tag and effective-price routing (P3). Phase 4 (bulk-edit modal + first-time onboarding banner) pending. P3 pending owner verification. Cart lines in QuickSale capture the price at first tap — they don't re-flip if the window ends mid-checkout. Quick Add chips (recent items) and manual freeform entry are intentionally NOT peak-aware.
