...
- **Peak Hour Pricing (#68)** — All four phases shipped: schema + Settings picker (P1), Canteen card + form field (P2), AddItemBottomSheet + QuickSale POS chips (P3), `BulkPeakPriceModal` + one-time amber onboarding banner + permanent "Bulk peak prices" header button (P4). Onboarding state lives in `localStorage('ck_peak_onboarding_seen')` — per-browser, one-time only, doesn't revive on toggle-off/on. QuickSale cart locks captured price across window edge. Quick Add chips + manual freeform entry intentionally NOT peak-aware. P4 pending owner verification — once verified, #68 ready to close.
