...
- **#122 (P1) — wrapper API gap BLOCKS the rest of the Chunk 7 cutover**: no mixed-op atomic batch (`syncedCreateBatch` is create-only). Until a `syncedBatch` supporting insert+update ships, the 8 multi-table tx sites in queries.ts (createCanteenSale, recordStockPurchase, updateSessionItem, deleteSessionItem, restoreSessionItem, createBackEntry) stay RAW — their writes do NOT sync. Do not split or nest them as a workaround (power-cut orphan / Pattern D7). Design proposal on the issue; needs owner go-ahead.
