...
- **Sync project (Phase B step 2 COMPLETE, 24 Jun 2026)** — Multi-device sync + staff login. Architecture doc at `references/sync_architecture_v2.md` (with v3.1 addendum + §5.6). Step 2 shipped (commit `ee40cda`): `.upgrade()` callback rewrites all numeric-id rows in `gameTables/sessions/sessionItems/canteenItems` to UUIDs atomically; remaps all FK fields including nested `Session.tableMoves[].fromTableId/.toTableId` (§5.6 landmine 2c); rewrites `canteenSales.items[].canteenItemId`, `stockPurchases.canteenItemId`, `bookings.tableId` via `.modify()`; adds `_migrationSeq` per row for Phase C resumable upload. All `number | string` unions collapsed to `string`. Dual-accept guards removed. Route-boundary parsers simplified to UUID-only. No pre-v20 backup (owner waived). Razorpay LIVE verified in production — auto-pay collecting ₹599. Next: Phase C (outbox drain + realtime sync).
