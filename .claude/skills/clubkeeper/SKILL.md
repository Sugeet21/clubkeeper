...
- **Sync project (Phase C Chunks 0–3 COMPLETE + VERIFIED, Chunk 4 UNBLOCKED, 26 Jun 2026)** — Multi-device sync + owner auth. DDL deployed; JWT hook patched (#108 closed) AND the RLS-on-users_meta-at-mint-time bug resolved (#109 closed — Pattern A9). Chunks 0–3 files: `syncWrappers.ts`, `syncTableMap.ts`, `scheduleDrain.ts` stub, `TestOutbox.tsx` DEV-only page, dev nav button in /settings. All wrappers atomic data+outbox writes in one Dexie tx; Pattern D7 top-of-file. JWT now carries `user_club_id` + `user_role` (verified by decoding a fresh access_token at jwt.io with `iat` post-policy). Next session: Chunk 4 (SyncRunner — actually hits Supabase).
