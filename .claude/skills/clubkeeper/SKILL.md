...
- **Peak Hour Pricing (#68)** — Phases 1–2 shipped: schema + Settings picker (P1) and per-item Peak price field in `CanteenItemFormModal` + two-price stacked card + amber "Peak · until X" header pill on Canteen with 60s tick (P2, `src/lib/peakPricing.ts`). Phases 3 (AddItem/QuickSale chips with `PEAK` tag) and 4 (bulk-edit modal + onboarding banner) pending. P2 pending owner verification.
