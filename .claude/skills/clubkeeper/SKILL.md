...
*Last updated: 17 Jun 2026 (Phase 1 advance booking — P1e-1 session linkage + prepaid advance shipped — pending owner verification — #84)*
...
**Pending:**
0. **Phase 1 advance booking — P1e-2 (remaining sub-phase of #84):** P1e-1 shipped 17 Jun 2026 (session linkage + walk-in conflict banner + prepaid-advance in PaymentSplitSheet — see Current State Snapshot). P1e-2 is the cancellation half: (a) player cancel button on `BookingScreen.tsx` visible when `status='confirmed' && now < slotStart − 2h`, hidden inside the 2h window; (b) new `supabase/migrations/20260617_booking_cancel.sql` with `cancel_booking_intent` SECURITY DEFINER RPC (slug + intentId + phone match for auth); (c) `cancelBookingIntent` (public) + `cancelBookingAndRefund` (owner) in `playerHubApi.ts` + `queries.ts`; (d) refund routes through realtime → owner Dexie → `creditBookingAdvanceRemainder` (already exists, reused with same `referenceType='booking_advance'`); (e) no-show auto-expire sweep — extend `App.tsx` `ExpirySweepRunner` (or sibling) to also scan `db.bookings` for `confirmed && slotEnd + 30min < now && !consumedSessionId` → mark `'no_show'`, no wallet credit (forfeit), same 4h cadence as coin expiry. Skill rules apply — update changelog + ripple_effects + bug_patterns (if new patterns) + Current State Snapshot in same session. Do NOT close #84 until owner verifies all of P1c/P1d/P1e-1/P1e-2 end-to-end on device.
