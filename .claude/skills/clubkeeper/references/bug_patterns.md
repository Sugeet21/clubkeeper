...
**Sweep query (Rule K):** `rg -n "\.bulkAdd\(|\.bulkPut\(|\.clear\(\)" src` — every hit outside `src/db/syncWrappers.ts` and `src/db/seed.ts` is a candidate local-only write; confirm it either doesn't hold synced domain data or it enqueues/backfills afterward.
**Files:** `src/lib/importEverything.ts` (the gap), reuse `src/db/backfillToSupabase.ts` (#129) + `src/db/syncWrappers.ts`.
