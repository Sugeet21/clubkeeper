...
- **Sites fixed in #139:** `authStore.signOut` (raced), `StaffSection.callStaffApi`, `Settings.handleCancelSubscription`, `Subscribe` create-subscription (all → `readAccessTokenLockFree()`). Still open: `playerHubApi.ts:144` `getUser()` → tracked on #103.
