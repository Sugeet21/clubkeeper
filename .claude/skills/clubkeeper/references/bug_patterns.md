...
**Note — soft-delete gap (open, tracked #166):** `Summary.canteenSalesForDate` does NOT yet filter `!deletedAt` (harmless today because no reverse-canteen-sale flow exists). #166 adds `reverseCanteenSale`; when it lands, EVERY canteenSales reader — including Summary + piggy — must add the `!deletedAt` filter or a reversed sale ghosts back into the totals (this is the S27/S29 reader-coverage law applied to canteenSales). `useCanteenSalesInRange` already filters it.
**Sweep query (Rule K):** `rg -n "db\.canteenSales\.where|useCanteenSalesInRange|getCanteenSalesByDate|canteenSalesForDate" src` — cross-check every revenue/report surface appears; `rg -n "db\.sessions\.where\('(startedAt|createdAt)'\)|useSessionsInRange|useSessionsForDate" src` — every hit that computes period revenue should have a sibling canteenSales read nearby.
