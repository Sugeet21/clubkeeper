...
**Rule:** Any mutation of a completed session's `startedAt`/`endedAt`/`pausedTotalMs` MUST recompute `amount` (+ `roundedDurationMs`) via the SAME path as `stopSession`: `billableMs = endedAt - startedAt - pausedTotalMs` → `applyRounding` (only when `!rateCard && billingMode==='per_hour' && rounding!=='none'`) → `calculateAmount(session, billableMs, rounding)`. Never write a completed-session time field alone.
**Second half of the rule (#163) — recomputing `amount` ALSO invalidates `paymentBreakdown`.** The invariant `paymentBreakdown.cash+upi+wallet === grandTotal` (types/index.ts) breaks the moment the total changes, and a stale split silently corrupts Summary PAYMENT MODE + piggy cash-in (they read `paymentBreakdown.cash`). So after the recompute, if the stored breakdown no longer sums to the new total, re-open `PaymentSplitSheet` and write the new split via `resplitSessionPayment(id, breakdown, customerId)`. That helper REVERSES the session's existing wallet leg before applying the new one — it computes `netDebited = Σ(session debits) − Σ(reversal credits)` for the `referenceId` from the append-only ledger, credits `netDebited` back, then charges the new wallet leg against the restored balance — so repeated edits never double-charge or drift. v1 scope: cash/UPI/wallet only; coin-redemption / booking-advance re-application at edit time is deferred.
**Files:** `src/db/queries.ts` (`editSessionStart` recompute + `resplitSessionPayment`), `src/pages/SessionDetail.tsx` (post-edit re-split sheet). Future edit-in-place paths (canteen line edits on a completed session) follow the same recompute-then-re-split discipline.
...
**Sweep query (Rule K):** `grep -rnE "where\('status'\).*anyOf\(\['running', 'paused'\]\)|status !== 'completed'" src/` — every hit is an active-session reader; confirm it filters `!deletedAt`. **FULL SWEEP 26 Jul (#189):** correct = `useActiveSessions` (useLiveData.ts:108), `getActiveSessions` (queries.ts:128), `useSessionAlarm` (reads filtered output). FIXED = `assertNoActiveSessions` (queries.ts, reset/clear guard), `importEverythingFromFile` active pre-check (importEverything.ts), `runImportExportRoundTrip` (dev). The earlier "errs safe — only over-blocks a wipe" note here was WRONG: over-blocking is the #189 bug — 8 soft-deleted `status='running'` tombstones made "Stop all active sessions before resetting" fire with RUNNING=0 on screen, so reset became impossible. Any status-only session reader without `!deletedAt` is this bug.
...
**A11 facet — the lock hangs ANY user-action auth call, not just boot (#139, 12 Jul 2026).** The #120 fix guarded BOOT (`initialize`). But every OTHER `await supabase.auth.getSession()` / `supabase.auth.signOut()` / `getUser()` in the app has the SAME infinite-hang exposure when the lock is stranded — and the symptom is nastier because there's no spinner, just a **dead button**: the code that updates the UI / redirects sits AFTER the hung await, so the click appears to do nothing until a manual hard refresh (which re-runs boot).
- **Symptom signature:** a button (Sign out, Create/Remove staff, Cancel/Pay subscription) does nothing on click; a hard refresh "fixes" it. Feels slow/broken. Owner reported this repeatedly before the root cause was pinned.
- **Rule:** NEVER `await supabase.auth.getSession()` just to grab a bearer token — use `readAccessTokenLockFree()` (reads in-memory authStore session, then a synchronous localStorage fallback; never touches the GoTrue lock). For `signOut()`, race the server-side revoke against a short timeout (`Promise.race([supabase.auth.signOut(), sleep(3000)])` in a try/catch) so the local teardown + `window.location.href='/'` ALWAYS run — a hung/failed server sign-out must never block the local sign-out (the token dies on its own ≤1h TTL, and the hard nav clears all client state).
- **Do NOT** substitute a lock-free token *refresh* — rule 4 above still holds (double-rotation revokes the family). Lock-free READ only. A near-expiry token → the server's `getUser(token)` returns a clean 401 → error toast, not a hang. Acceptable.
- **Detection grep:** `grep -rn "supabase\.auth\.\(getSession\|signOut\|getUser\)" src/` — each hit outside `authStore.initialize` (already guarded) and the lock-free helpers is a candidate. `getUser()` is worse than `getSession()` (network round-trip too) — the `playerHubApi.upsertClub` one is #103's slug-save freeze.
- **Sites fixed in #139:** `authStore.signOut` (raced), `StaffSection.callStaffApi`, `Settings.handleCancelSubscription`, `Subscribe` create-subscription (all → `readAccessTokenLockFree()`). **Last site fixed in #103 (19 Jul):** `playerHubApi.upsertClub` `getUser()` → lock-free JWT `sub` decode for `owner_id`. Sweep `grep -rn "auth\.getUser(" src/` = 0 remaining; the detection grep above should stay 0 outside `authStore.initialize` + the lock-free helpers.
...
### STANDING CHECK — a new field on a synced type must be wired into BOTH mappers, or it's silently local-only (promoted 22 Jul from #173/#174)
**This is a check to run every time, NOT a bug that happened.** Both `syncPayloadMapper.ts` (push) and `syncReadMapper.ts` (pull) are **strict whitelists** — they hand-map each field by name. A new field on a synced Dexie type (e.g. `StockPurchase.kind`/`reason` for #173) that is added to the TypeScript type but NOT wired into both mappers:
- is **silently dropped on push** (never reaches Supabase) — no error, because the whitelist just doesn't reference it. The local Dexie write already succeeded, so the failure is invisible on the authoring device.
- even if the Postgres column exists, it **never round-trips**: it stays local-only, and the gap **only surfaces on a second device** (which pulls the row without the field). On an offline-first synced app this is the worst kind of silent divergence.
**Rule for ANY new field on a synced type:** (1) confirm the Postgres column exists (`information_schema.columns` — if not, it's a migration, tracked separately, owner-run per Rule M); (2) wire it in `syncPayloadMapper.ts` (`if (row.newField !== undefined) out.new_field = row.newField`); (3) wire it back in `syncReadMapper.ts` (snake→camel on pull); (4) **cross-device check** — write on device A, confirm it appears on device B (not just that it landed in Supabase). Skipping (3) is the trap: push-only wiring passes a single-device test and a Supabase-row check, then breaks on the second device.
**Precedent to avoid re-deriving:** #159/#160 (missing `users_meta` → whole-club sync dead) and this class share a root — a value that exists locally but not in the synced contract. Grep sweep when auditing: for each `synced` type field, confirm it appears in BOTH `syncPayloadMapper.ts` and `syncReadMapper.ts`.
**Files:** `src/db/syncPayloadMapper.ts` (push), `src/db/syncReadMapper.ts` (pull), the synced type in `src/types/`.
...
### Pattern S23 — `supabase.removeChannel()` is ASYNC and `supabase.channel(topic)` reuses the old object until removal completes — teardown-before-register MUST await teardown (Phase C Chunk 5.4, 3 Jul 2026)
**Symptom signature (caught pre-commit, never shipped):** A channel-status-tracking feature (e.g. §7.4 polling fallback) that reacts to a channel's `CLOSED`/`CHANNEL_ERROR` status inside a teardown-before-register cycle (Pattern S22) can see a STALE status event for a channel that was intentionally torn down, arriving AFTER the replacement channel was already registered under the same generation. The stale event re-triggers "channel is down" logic for a group that is actually reconnecting cleanly — a false-positive that (depending on what the down-logic does) can start a poll loop, mis-render a status indicator, or worse.
**Root cause:** `supabase.removeChannel(channel)` returns a `Promise` — the actual websocket leave push + the channel's `_onClose` (which fires BOTH the internal `socket._remove(this)` deregistration AND the user's `CLOSED` status callback) resolve on a LATER TICK, not synchronously. Meanwhen `supabase.channel(topic)` (used to create the replacement) returns the EXISTING channel object if one with that topic string is still registered — and our channel topics are deterministic (`club:<id>:<group>`), so a teardown-before-register cycle under the SAME generation reuses the OLD channel's identity for the new registration until the async removal actually completes.
**Two insufficient fixes tried and rejected (in order), both defeated by the same async-removal fact:**
1. A boolean `tearingDown` flag set true before the removal loop and false immediately after — resets synchronously, before the deferred `CLOSED` fires. Useless.
2. Checking `!this.channels.includes(channel)` inside the status callback, reasoning that `teardownRealtime()` synchronously replaces the tracked channels array — defeated because `supabase.channel(topic)` hands back the SAME object, so the "new" channel pushed into the array IS the old one; `includes()` returns true when it needs to return false.
**Rule (the only sound fix):** Make `teardownRealtime()` return `Promise.all(channels.map(c => supabase.removeChannel(c)))` and have the re-subscribe path `await` it BEFORE calling `supabase.channel(topic)` again. Once the removal Promise resolves, the topic is genuinely free and `supabase.channel()` returns a fresh object — at which point any identity or generation guard on the NEW object works correctly, and a stale `CLOSED` from the OLD object (which can still arrive later) is harmless because it's tied to an object no longer referenced anywhere live.
**Constraint this creates:** any caller on a synchronous-boundary (e.g. a React effect cleanup / `stop()`) that cannot `await` teardown must still call it, but as an explicit void-dispatch with `.catch` (not a bare unawaited call) — and must separately guarantee correctness via a generation bump BEFORE the teardown call, so any late stale event from that path is caught by the ordinary generation guard instead of relying on the teardown having completed.
**Files affected:** `src/db/syncReader.ts` (`teardownRealtime`, `subscribeRealtime`, `stop`, `initialPull`).

### Pattern S24 — A read-dependent atomic multi-table op needs `syncedBatch(tables, fn)` (callback form), NOT an ops-list built before the tx (#122, Phase C Chunk 7, 7 Jul 2026)
**Symptom signature (design-time; caught at grounding):** When converting a `db.transaction('rw', ...)` that does read → check → write (stock sufficiency check → decrement, wallet balance check → debit) to a syncable batch, the tempting API is `syncedBatch(ops: {table, op, row}[])`: caller does the reads in plain code, builds the ops list, passes it. The bug: the read now happens BEFORE the tx opens, so a concurrent writer can slip between the check and the decrement — the sufficiency guarantee that the original single flat tx provided is silently lost. It also fails "preserve semantics byte-for-byte" for a plumbing swap.
**Root cause:** Atomicity of a check-and-mutate requires the read and the write to be in the SAME tx. An ops-list wrapper computes its ops (which depend on reads) outside the tx it will later open, so the read and write are in different tx scopes (or no tx at all for the read).
**Rule:** For any atomic multi-table op whose writes depend on reads, use `syncedBatch(tables, fn)` — the callback form (`src/db/syncWrappers.ts`, #122). The wrapper opens ONE tx over the caller-declared `tables` + `_outbox` and runs `fn(b)` INSIDE it; the caller does its reads directly on `db.*` (they auto-join the ambient tx) and emits ops via `b.insert/update/softDelete`, which write the data row + its outbox row together. `scheduleDrain()` fires once after commit. Constraints:
...
**Rule:**
1. Before writing a role-scoped policy on a synced table, read the table's mapper in `syncPayloadMapper.ts` and constrain on the values it ACTUALLY sends — never on DDL comments or design-doc enums. **This applies to EXCLUSION lists too (#132, found 11 Jul verifying the fix):** the fix migration excluded `reference_type IN ('adjustment','refund','reversal')`, but the app's real manual-adjustment shape is `reference_type='manual'` (`customerStore.ts:235`; `'adjustment'`/`'reversal'` are strings the app never sends) — so the forbidden write passed RLS. Blocked and allowed values BOTH come from the wire contract (`src/types/walletTransaction.ts` unions + the mapper), and the negative proof must probe the REAL app shape of the forbidden op, not the shape the policy author imagined.
...
### Pattern S27 — Local destructive op ignores the server mirror: "deleted" data resurrects on the next pull (#154, 19 Jul 2026)
**Symptom signature:** Owner deletes/clears/resets something, it vanishes from the UI — then a hard refresh (or next sign-in / next device) brings it all back. The op "worked" locally; sync faithfully restored it.
**Root cause:** Pre-Phase-C destructive helpers (`resetEverything`, `clearAllSessions`) operate on Dexie only. Post-Phase-C every data table mirrors to Supabase, and clearing `db.settings` also destroys the pull cursors (stored on settings row 1) — so the next boot's SyncReader pulls from epoch and re-hydrates everything. Compounding constraint: prod has ZERO DELETE policies (deliberate — clients never get raw DELETE), so a naive client-side `.delete()` "fix" silently removes 0 rows.
**Fix (SHIPPED #154):** owner-gated SECURITY DEFINER RPC `reset_club_data()` (migration `20260719_reset_club_data_rpc`) deletes the club's rows from the 9 synced tables + both player postboxes, children before parents, returns per-table counts; `clubs` row + `subscriptions` preserved. Client (`Settings.handleReset`) runs PH2 write order: `assertNoActiveSessions()` guard → `resetClubDataRemote()` → local `resetEverything()`; server failure aborts with red toast, clears nothing.
**Rule that generalizes:** ANY destructive op on synced data (reset, clear, bulk delete, import-replace) MUST have a server-side counterpart executed FIRST (PH2), or the delete is an illusion. Client-side `.delete()` is never that counterpart — there are no DELETE policies; go through an owner-gated RPC.
**Known v1 limit:** other devices holding a local copy keep it until their own re-pull cycle; LWW is upsert-only, deletes don't propagate device-to-device.
**Sweep query (Rule K):** `rg -n "\.clear\(\)|bulkDelete|\.delete\(\)" src/db src/pages src/components` — every hit on a SYNCED table outside the SyncReader/import internals needs a server-side counterpart or an explicit local-only justification. Swept 19 Jul 2026: `clearAllSessions` (Settings "Clear session history") is the 1 other user-facing instance → SWEEP #155 (also orphans session_items locally, pre-existing).
**SWEEP #155 SHIPPED 23 Jul 2026 (migration `20260723_clear_club_sessions_rpc`, CODE DONE pending owner verify):** same fix shape as #154 — owner-gated SECURITY DEFINER `clear_club_sessions()` (scoped sibling, session_items + sessions only, soft-delete not hard-delete since sessions/session_items readers all filter `!deletedAt` and a hard delete can't propagate to other devices via the apply-only pull path). `Settings.handleClearSessions` runs the same PH2 order as `handleReset`: RPC first, Dexie `clearAllSessions()` mirror only on success, abort+toast on failure. `clearAllSessions()` itself now soft-deletes BOTH `db.sessions` and `db.sessionItems` in one rw tx (closes the child-table-orphan half of the bug) — it does NOT enqueue outbox rows, since the RPC already wrote the server side; this is a local mirror, not an independent write. Re-ran the `\.clear\(\)` sweep post-fix: 0 more instances (remaining hits are the UUID-migration in `database.ts`, `resetEverything`/`importEverything.ts` full-DB ops, and dev-only `TestOutbox.tsx` — all pre-existing exceptions, not this pattern).

### Pattern S28 — An OWNER's `users_meta` row must be auto-provisioned, or sync is silently dead for every real customer (#159 #160, 19 Jul 2026)
**Symptom signature:** A real customer (esp. multiple partners sharing ONE login) reports "I can't see my data on the other device / sync failed." No error that points at the cause. Prod check: ZERO rows in every synced table across ALL clubs — no customer data ever reached Supabase. Each device holds its own local-only IndexedDB.
**Root cause:** Cross-device sync partitions on the `user_club_id` JWT claim, minted by `add_user_meta_to_jwt`, which reads it from `public.users_meta`. NOTHING provisioned an OWNER's users_meta row: `handle_new_user()` writes profiles + subscriptions only; `upsertClub()` writes the clubs row only. The only users_meta writers were the staff-create endpoint and ONE hand-inserted owner row (sugeetjadhav@gmail.com, #109) — which itself pointed at the WRONG club (#160). No users_meta row → hook returns the JWT unmodified → no claim → SyncReader defers forever (NoUserClubIdClaimError), SyncRunner can't stamp club_id → outbox dead-letters. The "Phase C sync closed / single-owner-multi-device proven" claim was only ever exercised on that one hand-patched account, so the systemic gap was invisible.
**Why trial mode is a red herring:** NOTHING in the sync path (hook, SyncReader, SyncRunner, RLS) reads subscription status. Sync is gated ONLY on the claim. Staff sync while the owner is `trialing`. Do not chase subscription state for a "sync not working" report — check `users_meta` + the claim first.
**Fix (SHIPPED #159/#160, migration `20260719_provision_owner_users_meta`):**
1. `AFTER INSERT ON public.clubs` trigger `on_club_created_provision_owner_meta` → `provision_owner_users_meta()` (SECURITY DEFINER) inserts `{user_id: owner_id, club_id: NEW.id, role:'owner', active:true, name: profiles.display_name ?? email, username: email}` with `ON CONFLICT (user_id) DO UPDATE` (one owner ⇒ one club, owning club authoritative). **`users_meta.name` is NOT NULL — source it or the insert 23502s.**
2. Backfill provisions every pre-trigger owner; #160 repoints sugeetjadhav@gmail.com to his own `clubkeeper` club.
3. Client (`upsertClub`, INSERT branch only) calls `supabase.auth.refreshSession()` after creating the club so the fresh JWT picks up the just-created claim — otherwise the device syncs blind until the ~1h auto-refresh. Best-effort (swallow failure; club row IS created; next auto-refresh still lands the claim).
**The club row is created ONLY at Player-Hub slug setup** (`upsertClub`) — NOT at signup, NOT when tables/canteen/stock are added. A customer who added inventory but never set a slug has no club row → no claim → no sync. This is the #1 support-triage question.
**Rule that generalizes:** any JWT-claim-derived partition key MUST have a server-side auto-provisioning path (trigger/RPC), proven live via `pg_trigger` + a real row check (Rule M) — never a hand-inserted row per account. And the client must force a token refresh at the moment the provisioning row is created, or the current device won't see the claim until the next background refresh.
**Verify:** `select c.slug, u.email, (um.user_id is not null) has_meta, (um.club_id=c.id) maps_own from public.clubs c join auth.users u on u.id=c.owner_id left join public.users_meta um on um.user_id=c.owner_id;` — every row must be `has_meta=true, maps_own=true`.
**Files affected:** `supabase/migrations/20260719_provision_owner_users_meta.sql`, `src/lib/playerHubApi.ts` (`upsertClub` refresh), `src/db/syncClubId.ts` (the claim reader — unchanged, but its NoUserClubIdClaimError guidance now has a real auto-fix behind it).

### Pattern S29 — Reversing a completed session = tombstone + FULL undo in one batch; every completed-session reader must exclude the tombstone (#162, 20 Jul 2026)
**Symptom signature (design-time — prevented):** owner "deletes" a wrong old session, but the day's Summary/piggy still counts it, or stock isn't returned, or the wallet debit isn't refunded, or the delete un-does locally but resurrects on next pull (Pattern S27). Any of these = money numbers the owner can't trust, which is the exact fear the feature removes.
**Root cause:** a completed session has FOUR side-effects that all must reverse together: (1) it counts in every completed-session aggregate; (2) its `session_items` decremented canteen stock at add-time; (3) it may have debited a customer wallet (`type:'debit', referenceType:'session'`); (4) its cash feeds the derived piggy. Reversing only some = inconsistent books.
**Rule (`reverseSession(id, reason)` in queries.ts, owner-only):** ONE `syncedBatch(['sessions','session_items','customers','wallet_transactions','canteen_items'])`:
1. Tombstone the session via `b.update` carrying `deletedAt` + `deletedBy` + `deleteReason` together (NOT `b.softDelete`, which only sends `deletedAt` and drops the audit). Soft-delete each non-deleted `session_item`.
2. Stock return: re-add each line's qty to the matching canteen item (`findMatchingCanteenItem` then normalized-name fallback). If the item was removed from the menu → un-delete it (`deletedAt:null`, `revertedStockAt` badge) to hold the stock; if gone entirely → re-create it badged. `stockEnabled===false` present → no stock op.
3. Wallet: if `paymentBreakdown.wallet>0`, find the `referenceType:'session'` debit row for `referenceId===sessionId`, write a `type:'credit', referenceType:'reversal'` row + restore `walletBalance`. NEVER soft-delete a wallet row (append-only, S24 rule 4).
4. Piggy self-corrects — no explicit piggy write. `getPiggyBalance` derives cash-in from completed sessions with a paymentBreakdown, and step 1's tombstone + the `!deletedAt` reader guard drop this one.
5. Audit `deletedBy` = owner uid, decoded lock-free from the JWT `sub` BEFORE the batch (no auth/network inside a syncedBatch — S24 rule 3). Idempotency guard: `session.deletedAt` already set → throw (no double-reverse).
**The reader-coverage law (Pattern S27 twin):** EVERY reader of session rows where a reversed session would matter must filter `!s.deletedAt`, or the tombstone resurrects in that surface. As of #162 the guarded set is: `useLiveData` (useActiveSessions/useTodaysSessions/useSessionsBetween/useSessionsForDate/useSessionsInRange), `getPiggyBalance`, `getTodaysSessions`, `getSessionsBetween`, `getSessionsWithBreakdownByDate`, `Summary.tsx` (4 direct `db.sessions.where` queries), `Home.tsx` todayStaticTotals, **and `createBackEntry`'s overlap check** (a reversed session keeps `status:'completed'` + its old start/end, so without the filter it still "occupies" its slot and blocks re-entering the corrected session — this gap was caught in review and would have broken the reverse-then-re-enter recovery flow the feature promises). `useSession(id)` single-get is DELIBERATELY unguarded (SessionDetail must still DISPLAY a reversed session's audit). `getAllDataForExport` is DELIBERATELY unguarded (backup must preserve tombstones for round-trip).
**Un-delete sync gap fixed:** the `canteen_items` push mapper originally only sent `deleted_at` when non-null — so re-creating a removed item (`deletedAt:null`) wouldn't clear the server tombstone. Now it sends `deleted_at:null` explicitly. Any future "un-delete" path on a synced table must verify its mapper propagates the null.
**Sweep query (Rule K):** `rg -n "db\.sessions\.where\('(startedAt|endedAt)'\)" src` — every hit that feeds a total/report needs `.filter(s => !s.deletedAt)`.
**Files:** `src/db/queries.ts` (`reverseSession`, `SessionReversalError`, reader guards), `src/db/syncPayloadMapper.ts` + `syncReadMapper.ts` (audit + canteen un-delete), `src/hooks/useLiveData.ts`, `src/pages/{Summary,Home,SessionDetail,Canteen}.tsx`, `src/types/index.ts` (`deletedBy`/`deleteReason`/`revertedStockAt`), `src/types/walletTransaction.ts` (`'reversal'`), migration `20260720_session_reversal_audit`.
**Canteen-sale twin (#166) — `reverseCanteenSale(saleId)`:** same shape for a walk-in Quick Sale (`canteen_sales`). Differences from the session version: (1) sale items are INLINE (`sale.items[]`, no separate table / no per-line soft-delete) — the SALE row itself is soft-deleted via `b.softDelete('canteen_sales')` (NOT `b.update` — there's no `deletedBy`/`deleteReason` column on canteen_sales; WHO is captured server-side by the `zz_stamp_actor` `updated_by` trigger; a delete-reason is deferred pending a column). (2) Stock-return match prefers `line.canteenItemId` (canteen sale lines carry it) before the `findMatchingCanteenItem`/name fallback. (3) Wallet debit to reverse is `referenceType:'canteen_sale'` (vs `'session'`). Reader-coverage law → Pattern S30's soft-delete set. `'reversal'` notes prefix = "Reversal of walk-in sale …".

### Pattern S30 — `canteenSales` is a SECOND revenue source; every date/range money surface must read it too (#93 → #141 → #165)
**Symptom signature:** A walk-in Quick Sale is recorded and succeeds, but a revenue surface (dashboard tile, Home today-strip, History list, day total, CSV) shows the money missing or the total understated. The owner reports "the sale isn't maintained" on that page.
**Root cause:** Walk-in Quick Sales are written to the `canteenSales` table by `createCanteenSale` — they are NOT `sessions` and carry no `tableId`. Any surface that queries only `db.sessions` (or `useSessionsInRange`/`useSessionsForDate`/etc.) to compute a day/range's revenue silently drops every walk-in sale. This has recurred THREE times: #93 (Summary, 4 places), #141 (Home today-strip), #165 (History list + day total + CSV).
**Rule:** Before adding OR editing any surface that shows/sums revenue for a day or range, ask "does this need walk-in sales?" — if it's money-for-a-period, the answer is almost always YES. Read `canteenSales` alongside sessions (`useCanteenSalesInRange` for a range, the inline `between(createdAt)` query for a single day as in Summary), filter `!deletedAt`, and fold `sale.total` into the total. Walk-in sales belong to NO table, so a per-table filter must EXCLUDE them (History's table dropdown does this).
**Known readers (keep this list current — it IS the coverage map):** `Summary.tsx` (`canteenSalesForDate` → revenue tile + PAYMENT MODE + piggy cashIn + rankTables synthetic row + topCanteenItems + bucketByHour), `Home.tsx` today-strip (#141), `History.tsx` (#165, via `useCanteenSalesInRange`). Piggy `getPiggyBalance` also reads `canteenSale.paymentBreakdown.cash` for cashIn.
**Soft-delete reader-coverage (CLOSED #166):** `reverseCanteenSale` (see S29 family) soft-deletes a walk-in sale, so EVERY canteenSales money reader MUST filter `!deletedAt` or a reversed sale ghosts back into totals (S27/S29 law applied to canteenSales). Guarded set as of #166: Home today-strip, Piggy cash-by-week, Summary ×4 (main revenue / per-date deltas / `canteenSalesForDate` live / cash aggregate), `getPiggyBalance`, `getCanteenSalesByDate`, `useCanteenSalesInRange`. **DELIBERATELY raw:** `getAllDataForExport` (backup preserves tombstones — round-trip). Any NEW canteenSales reader follows this rule.
**Sweep query (Rule K):** `rg -n "db\.canteenSales\.where|db\.canteenSales\.toArray|useCanteenSalesInRange|getCanteenSalesByDate|canteenSalesForDate" src` — every hit that feeds a total/report needs `!deletedAt` (except export). Cross-check every revenue/report surface appears; `rg -n "db\.sessions\.where\('(startedAt|createdAt)'\)|useSessionsInRange|useSessionsForDate" src` — every hit that computes period revenue should have a sibling canteenSales read nearby.
**Files:** `src/pages/{Summary,Home,History}.tsx`, `src/hooks/useLiveData.ts` (`useCanteenSalesInRange`), `src/db/queries.ts` (`getCanteenSalesByDate`, `createCanteenSale`), `src/types/index.ts` (`CanteenSale`).

### Pattern S31 — A local-only bulk write (import/restore) NEVER reaches Supabase; it must enqueue to the outbox or trigger the backfill (#188, 26 Jul 2026)
**Symptom signature:** Owner restores a backup (`Import everything`) — tables/sessions appear on this device, but on a fresh device / staff pull they're missing, and Supabase has none of them. Looks like "sync is broken" or "my data leaked". Also surfaces as a local-vs-server MISMATCH: `Reset everything` blocks on "stop active sessions" (it sees the imported running sessions locally) while the Tables page shows a different count (post-clear, server-pulled). Reported 26 Jul after importing `clubkeeper-backup-2026-07-22.json`.
**Root cause:** `importEverythingFromFile` (src/lib/importEverything.ts) does a raw Dexie transaction — `db.<store>.clear()` then `db.<store>.bulkAdd(rows)` across all 10 stores — writing DIRECTLY to IndexedDB with NO `syncedBatch`/`syncedCreate` and NO `backfillToSupabase` trigger. The sync engine only ever pushes rows that pass through the OUTBOX; a raw Dexie write is invisible to it. So imported rows are local-only by construction and can never propagate. (Contrast: `addTable`→`syncedCreate('game_tables',…)` and every normal mutation goes through the outbox and syncs correctly — the engine is fine; IMPORT is the one write path that skips it.)
**Rule:** Any code path that bulk-writes domain rows to Dexie OUTSIDE `syncWrappers` (import, restore, a future migration/seed of REAL data, a bulk backfill) MUST, after the local write, either (a) enqueue each row to the outbox, or (b) invoke the one-time `backfillToSupabase` path (#129) that uploads pre-outbox rows (`ignoreDuplicates:true` → ON CONFLICT DO NOTHING, required for append-only wallet_transactions). Respect: club-scoping (rows take the current user's `club_id` from the JWT claim), soft-delete semantics, append-only wallet rows. The demo SEED is the ONLY legitimate raw-Dexie bulk write that must NOT sync (it's device-local demo data for claim-less accounts — Pattern in seed.ts; and it's gated on `!jwtHasClubClaim()`).
**Sweep query (Rule K):** `rg -n "\.bulkAdd\(|\.bulkPut\(|\.bulkDelete\(|\.clear\(\)" src` for bulk writes, AND `rg -n "db\.(gameTables|sessions|sessionItems|customers|walletTransactions|canteenItems|canteenSales|stockPurchases|bookings)\.(add|put|update|delete)\(" src` for per-row writes — every hit outside `src/db/syncWrappers.ts` that touches synced domain data must either be a documented server-first mirror, an inbound sync-reader apply, a schema `.upgrade()`, the demo seed, or it must enqueue to the outbox / backfill afterward.
**Full sweep result (26 Jul 2026):** ONE genuine leak — `importEverything.ts:205-227` (#188). All other non-outbox writes are DELIBERATE + documented: `seed.ts:91` (demo, gated on `!jwtHasClubClaim`, must NOT sync), `clearAllSessions` queries.ts:608 (local mirror after `clear_club_sessions()` RPC, #155), `resetEverything` queries.ts:660 (local mirror after `reset_club_data()` RPC, #154, PH2), `syncReader.ts:791` (inbound apply — pushing back would echo-loop), `database.ts:415-476` (v20 UUID `.upgrade()` id-rewrite). ZERO stray per-row `.add/.put/.update/.delete` on synced tables outside the wrappers. So the write-side sync discipline holds everywhere EXCEPT import.
**Files:** `src/lib/importEverything.ts` (the gap), reuse `src/db/backfillToSupabase.ts` (#129 — the shipped "enqueue every local row to the outbox, ON CONFLICT DO NOTHING" path, purpose-built for exactly this) + `src/db/syncWrappers.ts`.
**FIXED (#188, 26 Jul 2026):** extracted the sentinel-free core of the #129 boot backfill into `enqueueAllLocalRowsToOutbox()` (backfillToSupabase.ts); `backfillLocalRowsToSupabase()` keeps ONLY the once-per-device sentinel guard and delegates (boot behaviour identical). `importEverythingFromFile` calls `enqueueAllLocalRowsToOutbox()` UNCONDITIONALLY after a successful restore-tx (a fresh import is new data that must upload regardless of any prior device sentinel; the core re-stamps `backfillEnqueuedAt` so boot won't double-enqueue). Best-effort: a push-enqueue failure sets `ImportSuccess.syncEnqueueFailed` (surfaced as a non-blocking "cloud sync pending" notice on the restore-success screen — never a silent no-sync) but does NOT fail the local restore. Runtime chain: enqueue insert-row (`ignoreDuplicates:true`) → `pushOne` → `.upsert(onConflict:'id', ignoreDuplicates:true)` = ON CONFLICT DO NOTHING (safe for append-only wallet_transactions). `settings` correctly NOT enqueued (device-local, not in `SYNC_TABLES_PULL_ORDER`). **Rule for any future non-outbox bulk write: call `enqueueAllLocalRowsToOutbox()` after it, or it won't sync.**
