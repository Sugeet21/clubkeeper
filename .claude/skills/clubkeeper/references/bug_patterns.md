...
**Files affected:** `src/pages/SessionDetail.tsx` (auto-resume effect, `handleConfirmStop` → `pauseForPayment`, `handleCancelPayment`, `confirmPaymentAndStop` call), `src/pages/Summary.tsx` (PAYMENT MODE filter), `src/db/queries.ts` (`getPiggyBalance` filter, `pauseForPayment`, `confirmPaymentAndStop`, `cancelPaymentAndResume`), `src/components/TableCard.tsx` ("Paying…" badge), `src/types/index.ts` (`paymentInProgress?` field).
