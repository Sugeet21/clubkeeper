...
## 26 Jul 2026 — #181 #182 #184 #185 #188 #189 MERGED to main + owner-closed; #183 (Edit End Time) built on staging

- **Promote-to-prod (Rule 16):** staging → main merge `1ffe24c` (`--no-ff`) shipped all six — #181 (cross-midnight back-entry), #182 (back-dated walk-in sale), #184 (delete-gate), #185 (staff read-view), #188 (import→outbox sync), #189 (active-session guard). Build clean on main. Vercel auto-deployed to `app.handbookhq.in`. Owner gave the go-ahead + close directive in one message → all six `gh issue close`d with the SHA.
- **#183 (BUG, P2) — History Edit-history could edit a session's START time but not its END.** Owner-found during #179 testing: if staff stopped a session at the wrong time, the owner had no way to correct the end → wrong duration + amount, uncorrectable. Fix mirrors edit-start exactly:
  - `queries.ts`: new `editSessionEnd(sessionId, newEndedAt)` (completed-only, not-future, `> startedAt`). Extracted `recomputeCompletedBill(session, startedAt, endedAt)` — the frozen `amount`+`roundedDurationMs` recompute — now SHARED by both `editSessionStart` and `editSessionEnd` so the two edit paths can never drift (identical to `stopSession` billing logic).
  - `SessionDetail.tsx`: completed-only "Edit End Time" button + edit-end `Modal`, both inside `<OwnerOnly>` (Pattern A12). Extracted the #163 re-split-reopen block into a shared `maybeReopenResplit()` (soft-delete-filtered item read) called by both edit handlers — a changed total re-opens `PaymentSplitSheet` for re-confirm. No top-bar pencil for end (rarer edit; action-list button only — deliberate, reviewer-noted).
  - Reviewer (Rule J, >100-LOC): **APPROVE, no violations** — billing recompute byte-for-byte matches stopSession, A12 gating on both trigger+mount, sync contract preserved. Build clean; strict tsc +2 over baseline (both the pre-existing #118 `session possibly-null` narrowing class on the two new `session.` accesses — same as adjacent handleSaveEditStart, NOT a real defect). On `staging`, pending owner verify then merge.
- **#190 (BUG, P1, owner-found while testing #183) — owner couldn't EDIT a completed session's ITEMS even with the Edit-history toggle ON.** #185 correctly made past-session items read-only (staff + owner-without-toggle), but the owner-WITH-toggle edit path was only ever wired for start/end time — the items sheet stayed "This session is closed. Items can't be changed." Root: `AddItemBottomSheet.tsx` hardcoded `isReadOnly = sessionStatus === 'completed'` with no override, and the History Edit-history toggle intent never reached SessionDetail (row tap navigated stateless).
  - Fix (39 LOC, main-thread review per Rule J): `AddItemBottomSheet` gained `allowCompletedEdit` → `isReadOnly = completed && !allowCompletedEdit`. `SessionDetail` reads `location.state.editHistory`, gates it `&& role==='owner'` (Pattern A12 — nav state never trusted alone; a staff device carrying the flag is still blocked), passes `allowCompletedEdit`, relabels the button "Edit Items", and re-opens the payment split via the existing `maybeReopenResplit()` (#163) on sheet close so PAYMENT MODE / piggy stay consistent when an item edit changed the total. `History` owner tap now passes `{ state: { editHistory: editMode } }`; staff `alwaysTappable` path stays stateless → read-only. Matrix: staff=view · owner+toggle-off=view · owner+toggle-on=edit. Data layer (`updateSessionItem`/`deleteSessionItem`) was already status-agnostic + stock-safe (#122/#124) — no query change.
  - **Follow-up (`aa21560`, owner-found on staging same day):** with the toggle OFF the owner's History rows were INERT (not tappable) — so the owner had no way to even VIEW a past session, only the edit path existed. Fixed: owner `SessionRow` is now `alwaysTappable` (like the staff #185 row) so every row opens; toggle OFF → read-only, ON → editable. And because owner rows now open with the toggle OFF too, the completed-session correction controls (Edit Start Time + top-bar pencil, Edit End Time, Delete Session) were re-gated from bare `<OwnerOnly>` to `<OwnerOnly>` + `canEditCompletedItems` — so a toggle-OFF open is a genuine read-only view. Active (running/paused) sessions still show Edit Start + Move regardless (`isActive || canEditCompletedItems`), since they're managed live from Tables. Build clean; strict tsc at 90 baseline (0 new).
  - On `staging`, pending owner verify (toggle OFF → tap a past session → View Items, no edit/delete buttons; toggle ON → Edit Items + Edit start/end + Delete all present).
- **Recovery note (this session):** an intermediate docs commit briefly captured a full deletion of `.claude/skills/clubkeeper/` (the directory vanished from disk mid-session — a zip/extraction artifact, `skills/` held only `clubkeeper_old (2).zip`). Caught immediately from the commit stat (3927 deletions, 0 insertions), `git reset --soft` + `git checkout HEAD -- .claude/skills/` restored every file from `3b0f303`; nothing was pushed. The three doc edits were re-applied fresh. No skill content lost.
...
## 26 Jul 2026 — #189 FIXED: reset/import active-session guard counted soft-deleted tombstones (Pattern S27/S29, 5th recurrence)

- **Bug (#189, owner-found):** "Stop all active sessions before resetting" fired while the Tables page showed RUNNING: 0 → reset impossible. Root: `assertNoActiveSessions` counted `status ∈ (running,paused)` with NO `!deletedAt` filter; staging DB had 8 soft-deleted `status='running'` tombstones (reversal/delete keeps the old status). `useActiveSessions` (Tables count) DOES filter, so the two disagreed.
- **Fix:** added `.filter(s => !s.deletedAt)` to `assertNoActiveSessions`. **Rule-K sweep caught 2 more:** `importEverythingFromFile`'s active pre-check (same missing filter — would wrongly block an import of a backup containing running tombstones) and the dev round-trip test. All 3 fixed. The T11 pattern note that had dismissed this as "errs safe" was corrected (over-blocking a wipe IS the bug). Build clean; 88-err baseline (0 new). On staging, pending owner verify (reset now runs; RUNNING count matches).

## 26 Jul 2026 — #188 FIXED: import/restore now syncs to Supabase (Pattern S31)

- **Bug (#188, owner-found while verifying on staging):** `importEverythingFromFile` restored a backup with raw `db.<store>.bulkAdd()`, bypassing the sync outbox → restored tables/sessions lived ONLY on the device, never reached Supabase. Owner proved it: after clearing site data, the local pull returned only the 1 genuinely-synced Snooker table; the imported ones were gone (never on the server). Rule-K sweep (b46de62) confirmed import was the SOLE non-outbox leak; every other raw write (seed/reset/clear/syncReader-apply/schema-upgrade) is deliberate.
- **Fix (3e1f334):** extracted `enqueueAllLocalRowsToOutbox()` from the #129 boot backfill (sentinel-free core — enqueue one `insert` outbox row per local row, `ignoreDuplicates`→ON CONFLICT DO NOTHING, FK-safe order, re-stamp sentinel, kick drain). `backfillLocalRowsToSupabase()` keeps ONLY the once-per-device guard + delegates (boot identical). `importEverythingFromFile` calls the core UNCONDITIONALLY after the restore tx (fresh import must upload regardless of prior sentinel). Best-effort: a push failure sets `syncEnqueueFailed` → non-blocking "cloud sync pending" notice on the restore-success screen (reviewer concern — no silent no-sync); local restore never fails.
- Runtime path verified end-to-end: enqueue → `pushOne` insert+`ignoreDuplicates` → `.upsert(onConflict:'id', ignoreDuplicates:true)` (safe for append-only wallet_transactions). `settings` correctly excluded (device-local). Build clean; strict tsc at 88-error #118 baseline (0 new). Reviewer APPROVED. On `staging`, pending owner verify (import a backup → confirm rows appear in Supabase → other device pulls them).
...
- **Server half:** `supabase/migrations/20260723_clear_club_sessions_rpc.sql` — new `clear_club_sessions()` RPC, owner-gated SECURITY DEFINER sibling of `reset_club_data()`, scoped to sessions + session_items only. SOFT-deletes (`deleted_at` + bumped `updated_at`, no hard DELETE) so the clear rides the normal LWW pull to every device — a hard delete would strand other devices since the pull path is apply-only and realtime DELETE can't propagate. Children (session_items) before parent (sessions). Rule M probe: `pg_proc` confirms SECURITY DEFINER + jsonb return; `pg_proc`/grants confirm `authenticated`-only execute, `anon`/`public` revoked. Owner self-check: Supabase Dashboard → Database → Functions → search `clear_club_sessions`. `src/lib/resetRemote.ts` — `clearClubSessionsRemote()` wrapper (+18, same `withTimeout`/15s/`ResetRpcResponse` shape as `resetClubDataRemote`).
- **Local half:** `src/db/queries.ts` `clearAllSessions()` rewritten from `db.sessions.clear()` to a bulk soft-delete of BOTH `db.sessions` and `db.sessionItems` (stamping `deletedAt`+`updatedAt`) in one Dexie rw tx — mirrors the server state, never enqueues outbox rows (the RPC already wrote the server; an outbox push would be a redundant re-write storm).
- **Settings.tsx:** `handleClearSessions` rewritten server-first, same PH2 idiom as the adjacent `handleReset` (#154) — RPC first; on failure, abort with a toast and do NOT touch Dexie; on success, mirror locally via `clearAllSessions()`. LWW-safe: remote `updated_at`=now, local `updatedAt`=now, tie → remote applies either way, same result.
- **Rule K sweep** (query: `grep -rn "\.clear()" src/ --include="*.ts" --include="*.tsx"`): 0 more local-only-wipe instances. The other `.clear()` sites are benign — `database.ts` UUID-migration (clear+rewrite-with-new-ids inside one tx), `resetEverything`/`importEverything.ts` (full-DB reset/import, `resetEverything` only runs after `resetClubDataRemote()` already succeeded server-side), `TestOutbox.tsx` (dev-only). Soft-delete-forgets-child-table sweep: both existing tombstone sites (`reconcileActiveSessions` auto-dedup, `reverseSession`) already route through the shared `returnSessionItemStock` helper, which soft-deletes session_items lines — neither forgets the child table.
- Build clean (98-line tsc baseline unchanged, #118/#138 debt — zero new errors, none in the 3 touched files). Commit: `fix(settings): #155 clearAllSessions server-first (refs #155 — pending owner verification)`.
