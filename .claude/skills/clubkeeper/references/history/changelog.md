...
## 23 Jul 2026 — #175 Chunk 0: insight rollup schema + pure re-derive + perf spike (plumbing only)

- **Scope guard (owner):** build ONLY the rollup plumbing now — no Canteen/Table insight pages until weeks of real restock data exist. This chunk = schema + pure re-derive + a perf spike, no display, no live-path writers. Runs in parallel with the real-club trial (it's device-local derived cache, trial-independent).
- **Schema (Dexie v24, additive, no `.upgrade()`):** new `rollups` store `'id, kind, day, [kind+day]'` — DEVICE-LOCAL, NOT-SYNCED derived cache (same class as `restockDrafts`). `DailyRollup` type (3 kinds: `item` = day×canteenItemId, `table` = day×tableId, `hour` = weekday×hour bucket). `CURRENT_SCHEMA_VERSION` 23→24 (backup shape stays `schemaVersion:21` — rollups never exported). Correctly ABSENT from the 9 synced tables, `getAllDataForExport`, `importEverything`, and both sync mappers.
- **`src/lib/rollups.ts` (NEW):** `deriveRollups(src, now)` is PURE (raw arrays → DailyRollup[], no Dexie/clock/random — `now` is a param) so it's testable + reproducible. `rebuildRollups(now)` is the thin Dexie wrapper: reads raw with `!deletedAt` filters, `clear()`+`bulkPut()` in ONE rw tx (wipe-safe, no half-built cache). `rollupId()` = deterministic `<kind>|<day>|<entityId>` so rebuild upserts (no dup accumulation). Day-key = date-fns `format(ts,'yyyy-MM-dd')` LOCAL (IST), matching Summary/History. `bump`/`NumericField` accumulator typing sums numeric fields with zero casts / no `any`.
- **Perf spike (`src/pages/__dev__/TestRollupPerf.tsx`, DEV-only route `/__dev/test-rollup-perf`):** synthesizes ~18 sessions + 25 walk-ins/day over N days, times the pure derive + the Dexie rebuild. ⚠ overwrites local raw tables (DEV only). Node bench (laptop, mirror of the pure derive) proved **linear scaling** (90d→730d: 11ms→88ms for ~4.7k→38k raw rows) and a **correctness check** (derived table revenue === Σ raw session amounts at every scale). On-device numbers are the owner's to capture; laptop 730d derive ≈88ms → low-end Android ≈300-450ms for a FULL rebuild (which Chunk 1 makes a fallback).
- **Spike findings (documented in-code for the display chunks):** (1) the `item` grain keys in-session lines by NAME and walk-ins by canteenItemId, so a product can live under two rollup ids — Chunk 2 must reconcile via `canteenMatch` before "top items"/"days of cover" display (else double-count). (2) `resetEverything` doesn't clear `rollups` — reviewer-flagged; add `db.rollups.clear()` to the reset tx in Chunk 1 when the first reader lands (harmless now, nothing reads it).
- Reviewer agent: **APPROVE**, no blocking violations; both concerns are correctly-deferred Chunk-1/2 items. Build clean; strict tsc at the unchanged 88-error baseline (0 new). On branch `feat/175-rollup-spike`, not merged.
- **Model LOCKED as built (owner, 23 Jul):** rollup shape frozen; on-device 730d perf test SKIPPED (not worth the data-wipe risk for a number unused until display pages exist — laptop linear proof stands). If display pages ever feel slow on a real phone, revisit Chunk 1 (incremental writers) THEN. **Safety hardening:** `TestRollupPerf` de-fanged — the destructive Dexie seed+`rebuildRollups` round-trip REMOVED; the harness now synthesizes in memory and times only the pure derive, so no route (dev or prod) can wipe local data. `rebuildRollups()` stays exported for Chunk 1. Branch PARKED (not merged, not deleted).
