...
- **#133** (fix 860e868, migration `20260711_server_actor_stamping` applied + E2E-verified incl. forge probe, awaiting owner close) — `created_by` was NULL on every synced-table row (no default/trigger, mapper never sends it) — zero server-side attribution of staff vs owner writes. Fixed with unforgeable `auth.uid()` BEFORE triggers on all 9 tables; also activates the syncReader equal-ms self-vs-peer tie-break. — see GitHub
