...
**Status:** D0–D6 complete (10 Jul 2026 — migration APPLIED; D2 endpoints 24/24; D3 role state + staff sign-in + RPC gate; D4 claim-gated seed + account-switch round-trip + staff Account card; D5 operations role gates — RoleGuard primitives, Pattern A12, first staff WRITE-path sync proof, plus one amendment: the Home Add-Table FAB IS gated despite the "Home: no gating" line below, because a table create is a staff-forbidden `game_tables` INSERT; D6 commerce role gates — Canteen/CustomerProfile/Piggy gated, UI proven both roles, **but the D6 runtime gate exposed #130/#131: staff top-ups and stock decrements 403 at RLS — the D1 policies don't match the wire contract (mapper sends Dexie `type` verbatim) nor upsert semantics (INSERT WITH CHECK applies to update-path upserts). Pattern S26. Fix migration `20260710_phase_d6_staff_write_rls_fix.sql` is paste-ready + UNAPPLIED; staff commerce E2E re-runs after the owner applies it.** Note for D9 step 5: the wire never carries `kind='adjustment'` for modern rows — the forbidden-403 proof should POST `kind='adjustment'` AND separately `kind='credit', reference_type='adjustment'`; both must 403). Tracking issue: **#128** — every Phase D commit refs it. Next: D7 (before D9). **⚠ D9 is BLOCKED by #129** (pre-Phase-C rows never backfilled — prod `game_tables` is empty, so D9 step 2 "pull populates real tables" cannot pass; needs its own backfill chunk first). Owner workaround meanwhile: re-save each table on the primary device.
