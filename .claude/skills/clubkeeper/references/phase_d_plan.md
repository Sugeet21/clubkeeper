...
**Status:** D0–D5 complete (10 Jul 2026 — migration APPLIED; D2 endpoints 24/24; D3 role state + staff sign-in + RPC gate; D4 claim-gated seed + account-switch round-trip + staff Account card; D5 operations role gates — RoleGuard primitives, Pattern A12, first staff WRITE-path sync proof, plus one amendment: the Home Add-Table FAB IS gated despite the "Home: no gating" line below, because a table create is a staff-forbidden `game_tables` INSERT). Tracking issue: **#128** — every Phase D commit refs it. Next: D6/D7 (any order, both before D9). **⚠ D9 is BLOCKED by #129** (pre-Phase-C rows never backfilled — prod `game_tables` is empty, so D9 step 2 "pull populates real tables" cannot pass; needs its own backfill chunk first). Owner workaround meanwhile: re-save each table on the primary device.
