...
- `src/components/settings/StaffSection.tsx` — owner-only Settings → Staff management (D8): list (Supabase `users_meta` owner-read, D1 policy) + create/reset/revoke via the D2 endpoints `/api/create-staff` + `/api/manage-staff`. Mounted inside `<OwnerOnly>` in `Settings.tsx`. Touches NO ClubSettings field (Rule H N/A for T2/R4/F5/S11/U10). Show-once password in component state only. List SELECT has a 12s `Promise.race` timeout → error-state-with-Retry (guards the #120 lock-stranded case). **Usernames (#157, supersedes deferred D8b):** `users_meta.username` (20260719 migration, backfilled; written by `api/create-staff.ts` — keep insert + column in lockstep) gives the list permanent usernames + a tap-to-copy login button on active rows; the PASSWORD stays unrecoverable (hash only — recovery is the reset_password endpoint, show-once). Show-once modal branches on explicit `mode: 'create'|'reset'`, NOT on username truthiness. Removed staff render as compact no-action record lines (#158) and are purged for good by `reset_club_data()` (#156 — see §Import/Export/Reset).
...
| Open a past session from History (#190) | Owner `SessionRow` is `alwaysTappable` (like the staff #185 row) → EVERY owner row opens `/session/:id`, carrying `{ state: { editHistory: editMode } }`. Toggle OFF → read-only view; toggle ON → editable. Staff `alwaysTappable` path stays stateless → always read-only. `SessionDetail.tsx` reads `location.state.editHistory` + gates it `&& role==='owner'` (A12 — nav state never trusted alone) → `canEditCompletedItems`. **On a COMPLETED session, ALL correction controls require `canEditCompletedItems`, not just `<OwnerOnly>`:** Edit Start Time (action button + top-bar pencil), Edit End Time (#183), Delete Session (#162/#184), and item add/edit/delete via `allowCompletedEdit` on `AddItemBottomSheet` (`isReadOnly = completed && !allowCompletedEdit`; button reads "Edit Items" vs "View Items"). Sheet close fires `maybeReopenResplit()` (#163) so the split re-confirms when an item edit changed the total. ACTIVE (running/paused) sessions always show Edit Start + Move for the owner (managed live from Tables) — those are gated `isActive || canEditCompletedItems`. Alarm pill + frame steppers already `isActive`-only. Data layer (`updateSessionItem`/`deleteSessionItem`) status-agnostic + stock-safe — no query change. Matrix: staff=view · owner+toggle-off=view · owner+toggle-on=edit. |
...
| Edit canteen item (name/price/stock) | **OWNER-ONLY (#169 — reverted 22 Jul, was staff-allowed 20 Jul)** — per-card Edit button + `CanteenItemFormModal` wrapped in `<OwnerOnly>`. Staff was silently overwriting `currentStock` via the modal's raw "Current stock" field (no audit trail). ⚠ **UI-GATE ONLY**: the `canteen_items` UPDATE RLS MUST stay open to staff because the in-session stock-decrement path (#131) depends on it — a hard DB block needs the decrement path split from arbitrary-count-edit (unfiled follow-up). For a UI-driven staff user the gate fully stops it. |
| Delete canteen item (soft-delete) | **OWNER-ONLY (#169 — reverted 22 Jul)** — per-card Delete button + delete-confirm `Modal` mount both in `<OwnerOnly>` (Pattern A12 mount-gate). Same shared-`canteen_items`-UPDATE-RLS caveat as Edit above → UI-gate only. |
| Restock (`stock_purchases` INSERT) | **OWNER-ONLY (#169 — reverted 22 Jul, was staff-allowed 20 Jul)** — per-card Restock button + `RestockSheet` mount in `<OwnerOnly>`. Unlike Edit/Delete this is a HARD block: migration `20260722_revert_staff_restock_rls` restored `stock_purchases` INSERT+UPDATE to owner-only (`user_role = 'owner'`), so a staff write 42501s even if the sheet were reached. Reverses `20260720_staff_restock_rls`. |
...
| Manage staff (create/reset/revoke) | `Settings.tsx` → `<OwnerOnly><StaffSection/></OwnerOnly>` (D8). Owner-only section; there is no staff-facing variant (staff never reach the owner Settings body — D4 split). Server side = D2 endpoints, owner-JWT-verified. |
...
⚠ **Staff read of the `clubs` row (#170, 22 Jul):** the UPI payment QR (SessionDetail post-stop, `QuickSale`, `WalletTopup`) needs `settings.upiId`, hydrated on a fresh/staff device by `getOwnerClub()` (`useSyncClubFromSupabase`) reading `clubs.upi_id`. The `clubs` SELECT policy was owner-only (`auth.uid() = owner_id`), so staff read 0 club rows → `getOwnerClub()` NULL → no QR. Migration `20260722_clubs_staff_select_rls` adds `clubs_staff_select` (`id::text = auth.jwt() ->> 'user_club_id'`). `getOwnerClub()` has NO `.eq()` filter — it relies on RLS — so the policy alone unblocks it; `.maybeSingle()` stays safe (no user ever sees >1 club row). `clubs` INSERT/UPDATE stay owner-only. Caveat: hydration only fills when `local.upiId === undefined` (useLiveData.ts) — a device that already stored an empty string skips the fill; owner types it in Settings or the guard needs the empty-string case (unfiled).
...
- **#162 session reversal — `reverseSession(id, reason)` (owner-only, Pattern S29):** a completed session is SOFT-deleted (tombstone on the SESSION row now, mirroring session_items) with a FULL undo — return stock, wallet credit-reversal, audit stamp — in one `syncedBatch(['sessions','session_items','customers','wallet_transactions','canteen_items'])`. **EVERY completed-session reader MUST now filter `!s.deletedAt`** (twin of the session_items rule above): `useLiveData` session hooks, `getPiggyBalance`, `getTodaysSessions`, `getSessionsBetween`, `getSessionsWithBreakdownByDate`, `Summary` (4 direct queries), `Home` todayStaticTotals. Raw BY DESIGN: `useSession(id)` single-get (SessionDetail displays the reversed session's audit), export/import. Piggy needs NO explicit reversal — it derives cash-in from completed sessions and the tombstone drops it. UI: SessionDetail "Delete Session" button + confirm modal, both inside `<OwnerOnly>` (Pattern A12). **History rows are inert by default (money-record protection); an owner-only "Edit history" toggle (`editMode` state in `OwnerHistory`, resets OFF every visit — never persisted) makes rows tappable → `navigate('/session/:id')` → the completed-session detail where Delete/Edit-Start live. Owner-only falls out of the existing `History` role split (staff get `StaffHistoryView`, no list).** Canteen list shows a "↩ reverted" badge for items whose stock returned from a reversal + menu re-creation (`CanteenItem.revertedStockAt`).
...
- **#173 Bulk restock (owner-only):** `src/pages/BulkRestock.tsx` (NumberPad-driven entry, draft R6a), `src/components/BulkRestockConfirmSheet.tsx`, `src/pages/RestockHistory.tsx` (batch list + reverse), `src/db/queries.ts` `confirmBulkRestock`/`listRestockBatches`/`reverseRestockBatch`, `src/lib/restockDraft.ts` (device-local, NOT synced). Chunk 6 printable sheet: `src/lib/restockItems.ts` (**shared ordering — see invariant**) + `src/lib/restockSheetPdf.ts` (lazy jspdf, A4 PDF download).
...
- **#167 — search boxes on the Canteen PAGE and QuickSale** (same `normalizeName` filter, same >6-item threshold as AddItemBottomSheet). Canteen: `search` state + `filteredItems` memo in `Canteen.tsx`; input between `StatsRow` (fed the UNFILTERED list — low-stock count is whole-canteen) and `ListArea` (fed `filteredItems` + `searchQuery` so it can show a "No items match" state distinct from the empty-canteen state). QuickSale: `search` + `filteredItems` in `QuickSale.tsx`; input above the items grid; cart is keyed by id and independent of the filter (filtering an item out does NOT drop its cart line). The shared `CanteenItemPicker` component (#167 part 2) is the next step — these two search boxes are the pre-extraction quick win.
...
- **#173 shared-ordering invariant (R1):** `listRestockItems()` in `src/lib/restockItems.ts` is the SINGLE source of "which items on the restock surface, in what order" (filter `isActive===true` + `stockEnabled===true`; **#176: order is now `compareRestockItems` = (categoryRank, name-lower, sortOrder-tiebreak)**, NOT bare `sortOrder`). BOTH `BulkRestock.tsx` (screen) and `restockSheetPdf.ts` (printed sheet) call it + number by array index, so screen row N === paper row N. **Never re-inline the ordering** — that reintroduces the divergence this helper exists to prevent. Scope: restock surface ONLY — `getCanteenItems()` (Canteen page/QuickSale/AddItem) stays on `sortOrder`. `restockSheetVersion(items)` = FNV-1a fingerprint (`<count>-<base36hash>` over ordered `id|name`) stamped on the PDF; deterministic, no clock/random.
- **#176 `CanteenItem.category` (synced, extensible):** union `drinks|cigarettes|snacks|other` but stored/synced as free-text (prod column `canteen_items.category` text/nullable/no-CHECK — live, NO migration). `categoryRank()`/`CATEGORY_ORDER` in `types/index.ts` = the one ordering source (unknown/null → last). **Both mappers wired** — push `syncPayloadMapper` explicit-null-clears (`row.category ?? null`); pull `syncReadMapper` is **LENIENT** (`typeof string`, NOT `reqEnum`) so a future value doesn't dead-letter older clients (standing check). ⚠ READERS: runtime value may be outside the union — validate via `categoryRank()`/`in CATEGORY_ORDER`, never `switch`-exhaustively. **Chunk 2 UI:** the ONE chip selector is `src/components/CategoryPicker.tsx` (chips derived from `CATEGORY_LABELS` in `types/index.ts` — same source as `CATEGORY_ORDER`; add a category ⇒ touch union + `CATEGORY_ORDER` + `CATEGORY_LABELS` together). Used by BOTH `CanteenItemFormModal` (add/edit, per-item) and `BulkCategoryTag` (`/canteen/tag-categories`, RequireOwner, one-time bulk pass — reads `getCanteenItems()` sortOrder for a stable list, does NOT change sell order, one Save writes only dirty rows). **Clearing a category:** the UI sends `category: undefined`; `updateCanteenItem` detects intent via **`'category' in patch`** (NOT truthiness) and writes a real `null` (generic param `Omit<CanteenItem,'category'> & { category: … | null }` so the null survives typing) → mapper emits NULL. An ABSENT category key leaves the stored value untouched. `addCanteenItem` includes category only when set (absent ⇒ NULL). To narrow a possibly-out-of-union stored value before display, use the local `knownCategory()` helper (mirrors `categoryRank`'s `in CATEGORY_ORDER` guard).
- **#173 `StockPurchase.kind`/`reason` (synced):** bulk-entry rows stamp `notes='bulk-entry <batchId> <ISO>'`; reversals write compensating rows `kind='reversal'`, `notes='bulk-reversal <batchId> <ISO>'`, `currentStock -= qty` clamped ≥0, never hard-delete. `kind` null/undefined MEANS `'received'`. **Both** `syncPayloadMapper` (push) and `syncReadMapper` (pull, enum-validated) are wired — a new synced field needs BOTH (bug_patterns standing check post-S14).
...
- `src/db/queries.ts` — `createCanteenSale`, `CanteenSaleInvalidError`, `CanteenSaleStockError(itemName, available)`, `getCanteenSalesByDate`, **`reverseCanteenSale(saleId)` + `CanteenSaleReversalError` (#166 — owner-only reversal, S29 twin; soft-deletes the sale + returns stock + reverses wallet leg in one syncedBatch; see bug_patterns S29 canteen-sale twin)**
...
Invariants:
- **Pattern S30 (canteenSales is a SECOND revenue source — any date/range money surface must read it):** walk-in Quick Sales live in `canteenSales`, NOT `sessions`. Every surface that shows or sums revenue for a day/range MUST query BOTH tables or it understates the total. Known readers: Summary (tile + PAYMENT MODE + piggy cashIn), Home today-strip (#141), History (#165), Piggy cash-by-week. Recurrence history: #93 → #141 → #165. Sweep query in bug_patterns S30 before adding any new revenue/list surface.
- **#166 — soft-delete reader-coverage:** `reverseCanteenSale` soft-deletes a walk-in sale, so EVERY canteenSales money reader ALSO filters `!deletedAt` (Home, Piggy cash-by-week, Summary ×4, `getPiggyBalance`, `getCanteenSalesByDate`, `useCanteenSalesInRange`). `getAllDataForExport` stays raw (backup keeps tombstones). Full list + the #162 Piggy-sessions residual it caught → bug_patterns S30/S29.
...
- → [History](#history) — **(#165)** walk-in sales render + total in History via `useCanteenSalesInRange`; **(#166)** reversible under the Edit-history toggle (`reverseCanteenSale`). Any change to `CanteenSale.total`/`subtotal`/`items` shape ripples to the History row + CSV.

Last updated: 21 Jul 2026 (#166 — reverseCanteenSale + full canteenSales soft-delete reader-coverage)
...
- `src/db/queries.ts` — `recordSessionPaymentBreakdown(sessionId, breakdown, customerId?)` (first-time record), **`resplitSessionPayment(sessionId, breakdown, customerId?)`** (#163 — SECOND writer of `paymentBreakdown` on a completed session; called when a time edit changed the total, re-opens `PaymentSplitSheet`). Re-split REVERSES the session's net wallet leg (`Σdebit − Σreversal` for the referenceId → credit reversal + restore) BEFORE applying the new wallet debit, so repeated edits never double-charge; the wallet customer is FIXED for the session's life (throws if a re-split tries to change who paid with a wallet leg — delete+re-enter instead). **Retroactive-piggy note:** changing `paymentBreakdown.cash` on an already-recorded session RETROACTIVELY shifts that day's piggy `cashIn` (§Piggy derives it from `paymentBreakdown.cash`) — correct, but flag it like the S29 reverseSession piggy note. `PaymentBreakdownInvalidError`, `WalletInsufficientError`. `WalletReferenceType 'reversal'` now has THREE producers: `reverseSession` (#162), `resplitSessionPayment` (#163), + `reverseCanteenSale` (#166); distinguished only by the `notes` prefix ("Reversal of session…" / "Re-split of…" / "Reversal of walk-in sale…").
...
Files in scope:
- `src/pages/Settings.tsx` — D4: the default export is a thin role wrapper (`useRole()` → staff renders `StaffAccountView`, owner renders `OwnerSettings`); **any new owner section goes inside `OwnerSettings`; the wrapper stays a single `useRole()` call with a branch** (Rules of Hooks). `StaffAccountView` = Account card only (name from `profile.displayName`; username = `user.email`, the §3.3 credential, shown by design; club from Dexie `settings.clubName` — the deferred staff-settings-sync gap means it renders the seed default until that ships) + Sign out. `OwnerSettings` (byte-identical to the pre-D4 component): `openSection: string` state (one open at a time; `''` = closed). `SettingsSection` (inline, not exported). Animation via `grid-rows-[1fr/0fr] opacity-100/0`. Sections in order: `club-info` (default open, holds name + currency one-liner + UPI + time-rounding), `tables`, `canteen` (low-stock + peak pricing — BUG-S5), `alerts`, `subscription`, `piggy`, `player-hub`, `staff` (D8 — owner-only via `<OwnerOnly>`, renders `<StaffSection/>`; see §Roles), `data`, `about`, `account`. Club Name + UPI Save use `SaveIndicator` (Pattern U10). The `staff` section deliberately does NOT use SaveIndicator (server ops with show-once success screens, not settings saves) and touches no ClubSettings field.
- `src/types/index.ts` — `ClubSettings` interface. `runawaySessionMinutes?` (#161, default 150, 0=off) lives here; read via `getRunawaySessionMinutes()`/`RUNAWAY_MINUTES_DEFAULT` (queries.ts) or `useDexieSetting`. Owner-only, NOT mirrored to Supabase.
- `src/db/queries.ts` — `getSettings`, `updateSettings`, `getRunawaySessionMinutes` (#161)
- **#161 runaway-session gate:** the `runawaySessionMinutes` field is consumed in `Home.tsx` (render-body filter of `activeSessions`, `status==='running'` + `getElapsedMs > threshold` + not-yet-orphaned; Pattern T4 — MUST stay in render body, never a memo/live-query) → red "still playing?" banner listing each over-threshold table, tap → `/session/:id`. Settings input in the **Alerts** section (typing-buffer variant, `handleRunawayBlur`, clamps 0 or 30–1440). If you change the field name, update `getRunawaySessionMinutes`, the Home filter, the Settings draft/blur, and `seed.ts` together.
- `src/db/seed.ts` — defaults. D4: the demo-tables `bulkAdd` is gated on `!jwtHasClubClaim()` (syncClubId) — claim ⇒ initial pull populates real tables; seeding would create ghost demo tables (staff first sign-in, owner second device). The settings singleton is ALWAYS seeded regardless (device-local; Dexie settings queries need the row). A new SAMPLE_* seed for a SYNCED table goes behind the same gate; device-local seeds must not.
...
Files in scope:
- `src/store/authStore.ts` — `initialize`, `onAuthStateChange`, `refreshProfile`, `signOut`; state `session, user, profile, subscription, role, loading, dbReady, subscriptionLoaded, authLockBlocked, _lastFetchedAt`. `initialize` races `getSession()` vs 8s timeout (#120, Pattern A11) — timeout → degraded boot from stored session; shared `mapProfileRow`/`mapSubscriptionRow` are the ONLY snake→camel mapping for profile/subscription (used by refreshProfile AND the degraded path — never fork them). **D3: `role` is set via `deriveRole(session)` in lockstep with EVERY `set({ session })` — any new session set-point MUST also set `role`** (four today: initialize normal, #120 degraded, onAuthStateChange, signOut reset). `refreshProfile` branches on role: staff → `get_club_subscription_status()` RPC mapped by `mapClubSubscriptionRow` (id `'club'`, userId `''`, razorpay nulls — synthesized, never written back); owner → the user_id-scoped `subscriptions` query as before. RPC row shape changes ripple to `ClubSubscriptionStatusRow` + `mapClubSubscriptionRow` here AND the RPC in the `20260710` migration.
- `src/hooks/useRole.ts` (D3) — `deriveRole(session)`: pure, lock-free JWT decode (Pattern S16 — NEVER `supabase.auth.*`); missing/invalid `user_role` claim on a live session → `'owner'` (staff always have `users_meta` ⇒ always have the claim; claim-less = legacy owner). `useRole()` reads `authStore.role` — role gates are render-time, no flicker. Circular-import note: this file ↔ authStore only reference each other inside function bodies — keep it that way. (Same TDZ caution for anything imported by `syncClubId.ts`: the authStore→syncRunner→supabaseSync→syncClubId cycle already produces a benign caught "initial Realtime auth token" warning at module eval — do not add module-top-level CALLS into this cycle.)
- `src/db/seed.ts` (D4) — demo-table seed gated on `jwtHasClubClaim()` (see §Settings entry). Ripple: if the JWT claim name `user_club_id` ever changes, update `jwtHasClubClaim` + `getOwnerClubIdFromJwt` + the hook migration together.
- `src/lib/authBootFallback.ts` — #120 lock-free boot fallback (Pattern A11): `readStoredSessionLockFree` (full-session localStorage read, ≥60s expiry runway), `fetchProfileAndSubscriptionRows` (plain-fetch PostgREST, Pattern S1 discipline; D3: `{skipSubscription}` opt for staff degraded boots — staff run degraded with `subscription` null + `subscriptionLoaded` true and re-check on recovery), `isAuthLockHeldByAnotherContext` (diagnostic). READ-ONLY by design — must never refresh tokens, write auth storage, or create a Supabase client.
- `src/components/signup/StaffSigninSection.tsx` (D3, mounted in `SigninForm`) — collapsed staff username+password sign-in, button onClick only (Critical Rule 7), Pattern A4 double-tap ref. Error mapping keyed on GoTrue messages: `'Invalid login credentials'` exact + `/not active|banned/i` (hook raise on `active=false` + ban both surface here) — if GoTrue ever rewords these, the mapping silently falls to the generic message (degraded UX, not broken). Success is handled entirely by `onAuthStateChange` → Signup's redirect effect; the component never navigates.
...
- **Rule:** Any new field in `authStore` that `useAccessGuard` reads MUST get a paired `*Loaded: boolean` flag — truthiness checks aren't safe (`undefined` vs `null` look identical). (`role` deliberately has NO `roleLoaded`: it resolves synchronously with `session` — there is no async window. `useAccessGuard` does not read it.)
- **D3 staff-gate invariants:** `useAccessGuard` reads the SYNTHESIZED club subscription for staff and needs zero role awareness — keep it that way; role-specific behavior lives in the pages (Subscribe staff card, Signup staff redirect). Staff must NEVER reach a payment action: Subscribe's staff early-return (blocked → renew card, usable → null while the guard bounces) is the enforcement point — any new payment CTA elsewhere needs its own role gate (D5–D7 build the primitives). Signup's redirect effect gates on `subscriptionLoaded` (A6) — do not remove that guard when editing Effect 2.
...
Last updated: 10 Jul 2026 (#128 D3 — `role` in authStore + `useRole` + staff sign-in + club-subscription RPC gate)
...
Files in scope:
- `src/pages/Subscribe.tsx` — `headline` `useMemo` discriminated union (`expired | early | welcome`), `visiblePlanIds` computation, handlers, `MONTHLY_PRICES`/`ANNUAL_PRICES`. D3: `role==='staff'` early-return (AFTER all hooks — keep it below the last `useEffect`) renders the "Ask the owner to renew" card for a blocked club sub, `null` for a usable one (guard effect bounces) — staff never see the Razorpay CTA.
...
Files in scope:
- `src/db/queries.ts` — `getAllDataForExport()`, `ClubKeeperBackupV16` interface (single source of truth — exported), `CURRENT_SCHEMA_VERSION` constant, `resetEverything()`, `assertNoActiveSessions()`, `ActiveSessionsPresentError`; `src/lib/resetRemote.ts` — `resetClubDataRemote()` → owner-gated `reset_club_data()` RPC (#154, Pattern S27). The RPC also purges REMOVED staff auth accounts (#156): triple-gated (caller's `club_id` AND `role='staff'` AND `active=false`), runs LAST in the tx so the no-cascade actor-stamp FKs are already clear; any residual FK violation aborts the whole reset atomically. Active staff + owner + `clubs`/`subscriptions` untouched. If you add an actor-stamp FK on a table reset does NOT delete, this purge can start failing — extend the wipe list or the purge together.
...
- `src/lib/playerHubApi.ts` — player RPCs (`submitBookingIntent`/`getBookingIntentStatus`/`cancelBookingIntent`/`getBookedSlots`) on `supabasePublic` + `withTimeout(..., 8000)`; owner ops (`getPendingBookings`, `confirmBookingIntent` returning the server ISO `confirmed_at`, `rejectBookingIntent`); `syncBookingConfigBySlug(BookingConfigPatch)` routed through `mirrorToSupabaseBySlug` (Pattern S11) and **THROWS on `!result.ok` (#97, PH2) — callers rely on the throw to abort their Dexie write; never demote it to warn-only**; `getBookedSlots` pre-migration-safe (returns [] on missing function).
...
- `src/components/PendingBookingsModal.tsx` — per-row confirm/reject state machine; confirm = Supabase FIRST via the #147 guarded `confirm_booking_intent` RPC → `syncedCreate('bookings', ...)` with `id = intent.id` and ConstraintError swallowed for idempotent retry; typed `intent_expired`/`slot_taken` failures render inline **'Slot taken — refund manually'** on the row (row stays visible, badge NOT decremented, Reject still available, no toast); reject = Supabase-only status flip (postbox boundary held); three render states rows/spinner/empty (Pattern M5).
...
- **Fresh-device hydration (#145):** the clubs row is mirror-only (NOT a Phase-C synced table), so the ONLY read path onto a fresh device is `useSyncClubFromSupabase` (useLiveData.ts) via `getOwnerClub`. It undefined-guard-backfills slug/topups/coins **and now `acceptsBookings`/`bookingOpenMinutes`/`bookingCloseMinutes`/`bookingAdvancePerSlot`**. RULE: if you add any owner-configurable clubs-row field, it needs BOTH a column in `getOwnerClub`'s `.select()`+DTO AND an undefined-guarded line here, or it silently resets to default on every fresh device/browser profile. Guard `!== null` for fields that are `number|null` on the DTO but `number?` in ClubSettings.
...
- **Lazy cleanup (#147-amended):** Each `submit_booking_intent` call deletes `non-pending AND created_at < now() - 24h AND slot_end < now()`. The `slot_end` clause is load-bearing — without it, confirmed bookings made >24h before their slot stopped blocking (double-book vector). No cron, no Pro plan. Free-tier safe.
- **Conflict check (#147-amended):** `submit_booking_intent` rejects overlap with any `confirmed` row or LIVE `pending` (created within 10 min) on same `(club_id, table_id)`; it also lazily flips this club's stale pendings to `'expired'` first. Player sees `slot_taken`.
- **10-min hold (D-Booking-2):** pending = soft hold, expires 10 min after creation unpaid/unreviewed. Lazy flips happen in submit (club-wide), `get_booking_intent_status` (own row), and `confirm_booking_intent` (target row). `get_booked_slots` is read-only (fires per picker load) — it filters stale pendings by age instead of flipping.
- **Confirm is guarded (#147):** owner confirm goes through `confirm_booking_intent(uuid)` — SECURITY INVOKER (RLS is the auth, same surface as the legacy client UPDATE), FOR UPDATE lock, idempotent on re-confirm (returns original `confirmed_at`), typed `not_found`/`intent_expired`/`slot_taken`. Client `confirmBookingIntent` maps these to `Error(<reason>)` and falls back to the legacy unguarded UPDATE ONLY when the RPC is not deployed yet (pre-migration window). Never reintroduce a bare client-side status UPDATE once the migration is live.
...
- **Canonical customer phone (#153, Pattern PH4):** `booking.playerPhone`/`topup.playerMobile` are bare 10 digits; `Customer.phone` is `'+91' + 10`. Every customer lookup/create fed by a player payload goes through `src/lib/phone.ts` (`toCustomerPhone`/`phoneLookupCandidates`/`preferCanonicalPhone`) — sites: `linkBookingToSession`, `reconcileCancelledBooking`, SessionDetail linked-customer effect. Write paths heal legacy bare-format rows to `+91` on touch. Never `.equals(booking.playerPhone)` raw.
...
- **Booked-slot visibility (#90, #147-amended):** `getBookedSlots` is anon-readable but exposes ONLY `(slot_start, slot_end, status)` — never `player_phone`, `player_name`, or `advance_amount`. Anyone scraping the slug sees timing + pending/confirmed only. The 8-day window cap blocks bulk harvesting. Filter is `confirmed` OR live `pending` (<10 min old); rejected/expired/cancelled/stale-pending rows don't block. Client defaults a missing `status` column to `'confirmed'` (pre-migration safe). BookingScreen renders pending holds as amber `Pending` chips + a "request pending — pick another slot" legend (`stepHoldStatus`; confirmed wins over pending on overlap).
