...
- `src/components/settings/StaffSection.tsx` — owner-only Settings → Staff management (D8): list (Supabase `users_meta` owner-read, D1 policy) + create/reset/revoke via the D2 endpoints `/api/create-staff` + `/api/manage-staff`. Mounted inside `<OwnerOnly>` in `Settings.tsx`. Touches NO ClubSettings field (Rule H N/A for T2/R4/F5/S11/U10). Show-once password in component state only. List SELECT has a 12s `Promise.race` timeout → error-state-with-Retry (guards the #120 lock-stranded case). **Username gap:** the list is name-only — `users_meta` has no email column and `auth.users.email` is unreachable via RLS, so the generated `<slug>.ck.local` username shows only on the create show-once screen. Permanent-username list = deferred D8b (a `list-staff` endpoint).
