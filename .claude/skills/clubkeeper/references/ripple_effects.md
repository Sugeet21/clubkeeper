...
| Edit completed-session ITEMS (#190) | Owner tap on a History row with Edit-history ON navigates `/session/:id` with `{ state: { editHistory: editMode } }` (`History.tsx`; staff `alwaysTappable` path stays stateless → read-only). `SessionDetail.tsx` reads `location.state.editHistory` + gates it `&& role==='owner'` (A12 — nav state never trusted alone) → `canEditCompletedItems`; passes it as `allowCompletedEdit` to `AddItemBottomSheet` (relaxes `isReadOnly = completed && !allowCompletedEdit`), relabels the button "Edit Items", and calls `maybeReopenResplit()` (#163) on sheet close so the split re-confirms when the item edit changed the total. Data layer (`updateSessionItem`/`deleteSessionItem`) is status-agnostic + stock-safe — no query change. Matrix: staff=view · owner+toggle-off=view · owner+toggle-on=edit. |
