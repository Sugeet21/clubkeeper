...
- → If a queries.ts mutation site is converted from raw `db.X.add/put/update/delete` to a wrapper (Chunk 7): the call MUST move OUT of any surrounding `db.transaction()` block, because the wrapper opens its own tx. If atomic multi-table behavior is required AND all writes are creates, use `syncedCreateBatch`. **If the atomic op mixes INSERT+UPDATE (the common case — see #122), the current wrapper API CANNOT express it: do NOT split into sequential wrapper calls (breaks the power-cut guarantee) and do NOT nest (Pattern D7). Leave the site raw and wait for the mixed-op `syncedBatch` (#122).**
- → Chunk 7 Group A state (4 Jul 2026, commit 8ff1e6d): game_tables + canteen_items single-table sites in queries.ts now go through wrappers (`addTable/updateTable/deleteTable/addCanteenItem/updateCanteenItem/softDeleteCanteenItem/bulkSetCanteenItemPeakPrices/decrementCanteenItemStock`). Still RAW (blocked on #122): `updateSessionItem`, `deleteSessionItem`, `restoreSessionItem`, `createBackEntry`, `createCanteenSale`, `recordStockPurchase` — all multi-table atomic txs. Any NEW single-table mutation on a synced table in queries.ts MUST use a wrapper from day one; any NEW multi-table atomic op should be flagged against #122.
...
Last updated: 4 Jul 2026 (Chunk 7 Group A — queries.ts wrapper cutover for game_tables + canteen_items single-table sites; #122 mixed-op batch gap documented, multi-table tx sites left raw)
