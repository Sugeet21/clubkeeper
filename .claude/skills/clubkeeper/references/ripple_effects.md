...
- **#175 insight rollups (derived cache — plumbing, Chunk 0):** `src/lib/rollups.ts` `deriveRollups()` (PURE: raw → `DailyRollup[]`) + `rebuildRollups()` (Dexie wrapper, `!deletedAt`, clear+bulkPut in one tx). Dexie v24 `rollups` store — **device-local, NOT synced, NOT exported** (like `restockDrafts`). It is a WIPE-SAFE index over raw sessions/session_items/canteen_sales/stock_purchases — raw stays source of truth. **Ripple:** any change to how those raw tables are read/shaped ripples into `deriveRollups`; nothing writes the cache on live paths yet (Chunk 1 adds incremental writers + must add `db.rollups.clear()` to `resetEverything`). Display chunks (2-5) BLOCKED pending real data. Chunk-2 caveat: the `item` grain keys in-session lines by NAME, walk-ins by canteenItemId — reconcile via `canteenMatch` before display or it double-counts. DEV perf harness: `/__dev/test-rollup-perf`.
- **#173 shared-ordering invariant (R1):** `listRestockItems()` in `src/lib/restockItems.ts` is the SINGLE source of "which items on the restock surface, in what order" (`orderBy('sortOrder')` + `isActive===true` + `stockEnabled===true`). BOTH `BulkRestock.tsx` (screen) and `restockSheetPdf.ts` (printed sheet) call it, so screen row N === paper row N. **Never re-inline the orderBy/filter** — that reintroduces the divergence this helper exists to prevent. `restockSheetVersion(items)` = FNV-1a fingerprint (`<count>-<base36hash>` over ordered `id|name`) stamped on the PDF for a future stale-sheet check; deterministic, no clock/random.
