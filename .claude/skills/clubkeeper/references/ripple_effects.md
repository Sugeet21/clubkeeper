...
| Restock (`stock_purchases` INSERT) | **STAFF-ALLOWED (owner-approved 20 Jul)** — `Canteen.tsx` per-card Restock button + `RestockSheet` mount ungated. RLS migration `20260720_staff_restock_rls` added a staff INSERT+UPDATE branch to `stock_purchases` (was owner-only); the `canteen_items` currentStock UPDATE already passed the staff policy. `recordStockPurchase` = `syncedBatch(['stock_purchases','canteen_items'])`; piggy source is a text field, no wallet write. |
