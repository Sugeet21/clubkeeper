...
Invariants:
- Pattern D7: stock logic in `updateSessionItem`/`deleteSessionItem`/`restoreSessionItem` is INLINED via `findMatchingCanteenItemForRow`. `updateSessionItem` runs one atomic `syncedBatch(['session_items','canteen_items'])` (#122, b1407e3 — session_items + conditional canteen_items updates). `deleteSessionItem`/`restoreSessionItem` stay RAW `db.transaction('rw', db.sessionItems, db.canteenItems, ...)` — deferred to #124 (hard-delete has no sync round-trip; needs a soft-delete model + reader `deletedAt` filters = out-of-scope semantics change). Zero calls to `decrementCanteenItemStock` or `addOrIncrementSessionItem` from inside any outer tx / batch callback. Both remain safe to call standalone.
...
- `src/pages/PlayerHubSettings.tsx` — "Accept bookings" toggle + "Advance per booking" input + "Accept topups" toggle. Toggle: Supabase-first via `syncBookingConfigBySlug` / `updateAcceptsTopups` (both routed through `mirrorToSupabaseBySlug`, Pattern S11). **Read-side invariant (after #97 fix):** three small `useEffect`s sync `acceptsTopups` / `acceptsBookings` / `advanceDraft` from `settings.*` whenever Dexie changes — Dexie is the single source of truth for owner-side reads. The mount-effect `getOwnerClub()` backfill ONLY runs when the Dexie value is `undefined` (fresh device); never overwrites existing Dexie state. Do NOT call `setAcceptsBookings(club.acceptsBookings)` unconditionally on remount — that's the #97 regression vector.
...
Files in scope (P1e-2 — shipped 18 Jun 2026):
- `supabase/migrations/20260618_booking_cancel.sql` (new, ⚠ pending manual run) — `cancel_booking_intent(p_intent_id uuid, p_player_phone text)` SECURITY DEFINER RPC. Phone match for authorization (no Supabase auth for player). Raises `not_found | invalid_status | too_late`. Status guard requires `'confirmed'`; window guard `now() < slot_start - interval '2 hours'`. Anon + authenticated execute granted.
- `src/lib/playerHubApi.ts` — `cancelBookingIntent({intentId, playerPhone})` via `supabasePublic` + `withTimeout(8000, 'cancel_booking_intent')`. Typed error mapping at the boundary.
- `src/pages/player/BookingScreen.tsx` — new `cancelling` + `cancelled` PageStates. `canCancel` derived (`slotStartMs !== null && now < slotStart − 2h`). Cancel button on the `confirmed` screen only when `canCancel`; inside the 2h cutoff the button is replaced with a static notice. Inline `cancelError` for typed errors from the RPC (Pattern F7, no toast). Cancelled screen shows "Your advance of ₹X has been refunded to your wallet."
- `src/lib/realtimeBookings.ts` — `BookingUpdateEvent {intentId, oldStatus, newStatus}` surfaced via new optional `onUpdate` callback in `subscribeToBookingIntents(clubId, onInsert, onUpdate)`. Single channel re-used; pending-count decrement logic unchanged.
- `src/components/BookingRealtimeBridge.tsx` — `handleUpdate` callback. When `oldStatus === 'confirmed' && newStatus === 'cancelled'` → `reconcileCancelledBooking(intentId)` + toast (unless owner on `/bookings`) "Booking cancelled — advance refunded to wallet" with Review → `/bookings`.
- `src/db/queries.ts` — `reconcileCancelledBooking(bookingId)`: single flat tx (Pattern D7), marks Dexie booking `'cancelled'` + lookup-or-creates customer by `playerPhone` + writes `WalletTransaction(credit, booking_advance, referenceId=bookingId)` for full `advanceAmount`. **Idempotent under realtime replay** — if booking already `'cancelled'` AND a `'booking_advance'` credit row with this `referenceId` already exists, early-exits. Missing Dexie row → silent noop.
...
Files in scope (P2 — shipped 22 Jun 2026, closes #106):
- `supabase/migrations/20260622_booking_hours_and_per_slot_advance.sql` (NEW, ⚠ pending manual run) — clubs gains `booking_open_minutes int`, `booking_close_minutes int`, `booking_advance_per_slot int default 50` (CHECK 0–2000) + `booking_hours_valid` CHECK (`(both null) OR (open in [0,1439], close in [1,2880], close > open)`). `get_club_public_info` dropped+recreated with 3 new OUT params. `submit_booking_intent` dropped+recreated to (a) reject when hours unset (`hours_not_set`), (b) recompute server-side advance as `ceil(duration_min/30) * coalesce(booking_advance_per_slot, 50)` and raise `advance_mismatch` on disagreement, (c) raise `outside_hours` for NON-OVERNIGHT clubs (`close ≤ 1440`) when slot's `Asia/Kolkata` minutes-since-midnight falls outside `[open, close-30]`. Overnight (close > 1440) clubs skip the server-side bounds check — client gates and per-slot recompute remain the safety nets.
- `src/types/index.ts` — `ClubSettings` adds `bookingOpenMinutes? | bookingCloseMinutes? | bookingAdvancePerSlot?`. `bookingAdvanceAmount` marked `@deprecated 22 Jun 2026` — retained for Dexie/Supabase column compatibility, never read for new bookings.
- `src/types/playerHub.ts` — `ClubPublicInfo` gains `bookingOpenMinutes / bookingCloseMinutes (number | null)` + `bookingAdvancePerSlot: number`. `bookingAdvanceAmount` kept on type with `@deprecated` JSDoc.
- `src/db/database.ts` — bumps to v19 (additive only, no `.upgrade()`, schema string identical to v18).
- `src/db/queries.ts` — `CURRENT_SCHEMA_VERSION = 19`. Backup interface renamed to `ClubKeeperBackupV19`; old `V18`/`V17`/`V16` aliases preserved as structural-typing back-compat.
- `src/db/seed.ts` — `DEFAULT_SETTINGS.bookingAdvancePerSlot = 50`. Open/close intentionally undefined so owner must explicitly set them before Accept Bookings can be enabled.
- `src/lib/validation.ts` — adds `isValidBookingHours(open, close)`, `canEnableBookings(settings)`, `minutesSinceMidnightLocal(ms, tz='Asia/Kolkata')` (used client-side; server re-checks non-overnight).
- `src/lib/playerHubApi.ts` — `syncBookingConfigBySlug(slug, patch)` signature changed from `(slug, acceptsBookings, advance)` to a `BookingConfigPatch` shape (only fields present are forwarded). Routes through `mirrorToSupabaseBySlug` (Pattern S11). Mapper in `getClubPublicInfo` extended for the 3 new RPC columns with `?? null` defaults — pre-migration safe. `submitBookingIntent` error mapping extended for `hours_not_set | outside_hours | advance_mismatch`.
- `src/pages/PlayerHubSettings.tsx` — booking card rebuilt. Opens-at / Closes-at 30-min-step `<select>`s (Opens 0–1410; Closes 30–1740, labels past 1440 = "h:mm AM/PM next day"). Accept Bookings toggle is gated on `canEnableBookings(settings)` — disabled with helper text "Set opening & closing hours first" until both are set. The legacy "Advance per booking" numeric input is replaced by "Advance per 30 mins" (0–2000); the previous `bookingAdvanceAmount` field is no longer touched by the UI but the Dexie value stays on the row. All four save sites use `useSaveIndicator()` + `<SaveIndicator>` (Pattern U10). All clubs-row writes route through `syncBookingConfigBySlug` → `mirrorToSupabaseBySlug` (Pattern S11). Read side respects Pattern R4 — open/close are read directly off `settings` (the hook only drives the write path because Dexie's undefined is the "not set" sentinel).
- `src/pages/player/BookingScreen.tsx` — `buildTimeOptions(date, now, openMin, closeMin)` settings-driven; 30-min steps from `openMin` to `closeMin - 30` inclusive. New `not_configured` PageState rendered when `bookingOpenMinutes === null || bookingCloseMinutes === null` — NO hardcoded 8 AM fallback (the entire point of #106). Overnight slots (start ≥ 1440 min) carry a small "Late-night" caption; v1 keeps them under the same calendar-day header. Advance computed as `slotsBooked * bookingAdvancePerSlot` where `slotsBooked = ceil(durationMin / 30)`. Summary shows breakdown "{N} × 30 min slots @ ₹{perSlot}/slot". `advance_mismatch` → inline "Pricing changed. Please retry." bouncing back to summary step; `outside_hours` → bounces back to time step; `hours_not_set` → not_configured state.
...
Last updated: 22 Jun 2026 (#106 — per-club hours + per-slot advance [R4, S11, U10, T1, P2])
...
- → Chunk 7 Group A state (7 Jul 2026): game_tables + canteen_items single-table sites converted in 8ff1e6d. #122 `syncedBatch` SHIPPED b1407e3 — 4 of the 6 formerly-raw multi-table atomic sites now converted: `recordStockPurchase`, `createCanteenSale`, `updateSessionItem`, `createBackEntry` (runtime-proven into prod Supabase — sale+wallet-debit+stock all atomic). Still RAW, deferred to #124 (NOT a plumbing swap — hard-delete has no sync round-trip, needs soft-delete model + reader `deletedAt` filters): `deleteSessionItem`, `restoreSessionItem`. Any NEW single-table mutation on a synced table in queries.ts MUST use a single wrapper from day one; any NEW mixed-op atomic op MUST use `syncedBatch`. **Next: Group B (sessions/sessionItems/customers/walletTransactions/bookings, ~45 sites) — most share the same mixed create+update shape and now use `syncedBatch`.**
