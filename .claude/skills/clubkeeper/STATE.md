...
Phase D (staff login, tracking issue #128) — D0–D2 done (plan `references/phase_d_plan.md`; matrix amended: staff keep back-entry creation, staff can create customers; `20260710` migration APPLIED; api/create-staff + api/manage-staff live in repo, runtime matrix 24/24 incl. staff-token 403 + trigger-skip proof); next chunk: D3 (role in auth state + staff login form + owner-subscription gate). RPC smoke for `get_club_subscription_status()` rides D3's gate. Phase D endpoints reach prod on next push to `main`. Phase C tail: #125/#126 verification outstanding. Immediate P1: #127 (player booking broken post-v20, migration held by owner).
