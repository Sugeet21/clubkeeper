...
**Last verified: 11 Jul 2026 (CLI session — D6 tail fully closed (#130–#133 owner-closed); D7 routes/nav/Summary role gates shipped, reviewer PASS, deep-link runtime proof lands with D9).**
...
Phase D (staff login, tracking issue #128) — D0–D7 done (plan `references/phase_d_plan.md`; matrix amended: staff keep back-entry creation, staff can create customers; `20260710` migration APPLIED; staff admin endpoints + client role state + staff sign-in + RPC gate + claim-gated seed + staff Account card + operations role gates + commerce role gates (Canteen/CustomerProfile/Piggy, Pattern A12) + route/nav/Summary gates (`RequireOwner` on `/piggy`, staff today-only Summary card, nav unchanged) all shipped; `20260710_phase_d6_staff_write_rls_fix` + `20260711_staff_manual_adjustment_rls_exclusion` + `20260711_server_actor_stamping` all APPLIED — staff commerce writes E2E-verified, #130/#131/#132/#133 closed — server actor stamping (`created_by`/`updated_by` = `auth.uid()` triggers, forge-proof) live in prod). Next chunk: D8 (Settings staff management — Rule H pre-flight mandatory). **D9 blocker: #129 — pre-Phase-C rows (incl. ALL of game_tables) were never backfilled to Supabase; needs its own chunk before D9.** D7 staff deep-link/today-card runtime proof folds into D9's two-profile run. Phase D code reaches prod on next push to `main`. Phase C tail: #125/#126 verification outstanding. Immediate P1: #127 (player booking broken post-v20, migration held by owner).
