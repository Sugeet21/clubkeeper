...
Phase C write-site cutover DONE (owner verification of #124/#125/#126 outstanding); next: Phase D (staff login) per `references/history/sync_architecture_v2.md` §2–3. Immediate P1: #127 (player booking broken post-v20).

## Module status (one line each — overwrite in place)

- **Sync (Phase C)** — Write path (outbox + SyncRunner on lock-free `supabaseSync`) + read path (SyncReader: serialized queue, direct-apply LWW on epoch-ms, 4 realtime channel groups, 30s-grace/60s polling fallback) LIVE across all 9 tables; write-site cutover 100% (#122/#126/#124 — incl. session-item soft-delete + `!deletedAt` filters on every session_items reader), runtime-proven (delete/undo/ghost-pull rounds, outbox 0); #124/#125/#126 pending owner verification. Contract: `ripple_effects.md` §Sync + Patterns S14–S24.
...
Hand-notes that survive regeneration: #110/#120/#124/#125/#126 have fixes shipped and only await Sugeet's device verification + "close #NN" (#124's runtime rounds already passed on his machine). #114 & #121 are duplicates — ask Sugeet to close one. #118 (vacuous tsc build gate) is the biggest regression risk in the repo and needs its own triage session.
