...
**Last verified: 10 Jul 2026 (CLI session, Phase D D5 close — operations role gates green; first staff write-path sync proof).**
...
Phase D (staff login, tracking issue #128) — D0–D5 done (plan `references/phase_d_plan.md`; matrix amended: staff keep back-entry creation, staff can create customers; `20260710` migration APPLIED; staff admin endpoints + client role state + staff sign-in + RPC gate + claim-gated seed + staff Account card + operations role gates (RoleGuard primitives, Pattern A12, staff write-path sync proven) all shipped); next chunks: D6/D7 (commerce + routes/nav/Summary role gates, any order, both before D9). **New blocker for D9: #129 — pre-Phase-C rows (incl. ALL of game_tables) were never backfilled to Supabase, so staff/second devices pull an empty tables grid; needs its own chunk before D9.** Phase D code reaches prod on next push to `main`. Phase C tail: #125/#126 verification outstanding. Immediate P1: #127 (player booking broken post-v20, migration held by owner).
